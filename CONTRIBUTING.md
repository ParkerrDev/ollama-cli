...
- **[Before you begin](#before-you-begin):** Essential steps to take before
  becoming a Ollama CLI contributor.
...
- **[Documentation contribution process](#documentation-contribution-process):**
  How to contribute documentation to Ollama CLI.
...
To run the review tool, enter the following command from within Ollama CLI:
...
labeled
["help wanted"](https://github.com/google-ollama/ollama-cli/issues?q=is%3Aissue+state%3Aopen+label%3A%22help+wanted%22).
...
with a value of `OLLAMA_API_KEY` and set that to a valid API key that you have
...
```bash
git clone https://github.com/google-ollama/ollama-cli.git # Or your fork's URL
cd ollama-cli
```
...
setting `OLLAMA_SANDBOX=true` in your `~/.env` and ensuring a sandboxing
provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See
[Sandboxing](#sandboxing) for details.

To build both the `ollama` CLI utility and the sandbox container, run
...
To start the Ollama CLI from the source code (after building), run the following
...
If you'd like to run the source build outside of the ollama-cli folder, you can
utilize `npm link path/to/ollama-cli/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias ollama="node path/to/ollama-cli/packages/cli"` to run with `ollama`
...
the Ollama CLI. They are not run as part of the default `npm run test` command.
...
- Consult
  [OLLAMA.md](https://github.com/google-ollama/ollama-cli/blob/main/OLLAMA.md)
...
- `packages/`: Contains the individual sub-packages of the project.
  - `a2a-server`: A2A server implementation for the Ollama CLI. (Experimental)
  - `cli/`: The command-line interface.
  - `core/`: The core backend logic for the Ollama CLI.
  - `test-utils` Utilities for creating and cleaning temporary file systems for
    testing.
  - `vscode-ide-companion/`: The Ollama CLI Companion extension pairs with
    Ollama CLI.
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/ollama.js` within the
...
ollama-cli due to automatic exclusion. Use `.ollama/.env` files for ollama-cli
...
1.  **Start the Ollama CLI in development mode:**
...
On macOS, `ollama` uses Seatbelt (`sandbox-exec`) under a `permissive-open`
...
`.ollama/sandbox-macos-<profile>.sb` under your project settings directory
`.ollama`.

#### Container-based sandboxing (all platforms)

For stronger container-based sandboxing on macOS or other platforms, you can set
`OLLAMA_SANDBOX=true|docker|podman|<command>` in your environment or `.env`
...
as you start/stop Ollama CLI. Files created within the sandbox should be
...
for your projects by creating the files `.ollama/sandbox.Dockerfile` and/or
`.ollama/sandbox.bashrc` under your project settings directory (`.ollama`) and
running `ollama` with `BUILD_SANDBOX=1` to trigger building of your custom
sandbox.
...
can be specified as `OLLAMA_SANDBOX_PROXY_COMMAND=<command>`, where `<command>`
...
- Open [an issue](https://github.com/google-ollama/ollama-cli/issues) to discuss
  your proposed changes.
- Reach out to the maintainers.

We appreciate your contributions to making Ollama CLI documentation better!
