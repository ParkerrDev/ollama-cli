...
if (!image.length) {
  console.warn(
    'No default image tag specified in ollama-cli/packages/cli/package.json',
  );
}
...
console.log('packing @google/ollama-cli ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-ollama-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w @google/ollama-cli --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing @google/ollama-cli-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-ollama-cli-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w @google/ollama-cli-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(cliPackageDir, 'dist', `google-ollama-cli-${packageVersion}.tgz`),
  0o755,
);
chmodSync(
  join(corePackageDir, 'dist', `google-ollama-cli-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  if (sandboxCommand === 'podman') {
    if (isWindows) {
      ...
      tempAuthFile = join(os.tmpdir(), `ollama-auth-${Date.now()}.json`);
  ...
  const imageTag =
    process.env.OLLAMA_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
