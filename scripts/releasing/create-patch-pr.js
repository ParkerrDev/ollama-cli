...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option('cli-package-name', {
      description:
        'fully qualified package name with scope (e.g @google/ollama-cli)',
      string: true,
      default: '@google/ollama-cli',
    })
    .option('dry-run', {
  ...
  run('git config user.name "ollama-cli-robot"', dryRun);
  run('git config user.email "ollama-cli-robot@google.com"', dryRun);
