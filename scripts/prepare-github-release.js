...
fs.writeFileSync(
  coreNpmrcPath,
  '@google-ollama:registry=https://npm.pkg.github.com/',
);
console.log('Wrote .npmrc for @google-ollama scope to packages/core/');

// Update @google/ollama-cli
updatePackageJson('packages/cli/package.json', (pkg) => {
  pkg.name = '@google-ollama/ollama-cli';
  pkg.files = ['bundle/'];
  pkg.bin = {
    ollama: 'bundle/ollama.js',
  };
...
// Update @google/ollama-cli-a2a-server
updatePackageJson('packages/a2a-server/package.json', (pkg) => {
  pkg.name = '@google-ollama/ollama-cli-a2a-server';
});

// Update @google/ollama-cli-core
updatePackageJson('packages/core/package.json', (pkg) => {
  pkg.name = '@google-ollama/ollama-cli-core';
});
