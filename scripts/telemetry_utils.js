...
// User-level .ollama directory in home
const USER_OLLAMA_DIR = path.join(os.homedir(), OLLAMA_DIR);
// Project-level .ollama directory in the workspace
const WORKSPACE_OLLAMA_DIR = path.join(projectRoot, OLLAMA_DIR);

// Telemetry artifacts are stored in a hashed directory under the user's ~/.ollama/tmp
export const OTEL_DIR = path.join(USER_OLLAMA_DIR, 'tmp', projectHash, 'otel');
export const BIN_DIR = path.join(OTEL_DIR, 'bin');

// Workspace settings remain in the project's .ollama directory
export const WORKSPACE_SETTINGS_FILE = path.join(
  WORKSPACE_OLLAMA_DIR,
  'settings.json',
);
...
export function getJson(url) {
  const tmpFile = path.join(
    os.tmpdir(),
    `ollama-cli-releases-${Date.now()}.json`,
  );
  try {
    const result = spawnSync(
      'curl',
      ['-sL', '-H', 'User-Agent: ollama-cli-dev-script', '-o', tmpFile, url],
...
export async function ensureBinary(
  ...
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'ollama-cli-telemetry-'),
  );
