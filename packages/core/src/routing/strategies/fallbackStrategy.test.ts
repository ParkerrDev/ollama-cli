...
describe('FallbackStrategy', () => {
  ...
  describe('when in fallback mode', () => {
    it('should downgrade a pro model to the flash model', async () => {
      ...
      expect(decision?.model).toBe(DEFAULT_OLLAMA_FLASH_MODEL);
    ...
    it('should honor a lite model request', async () => {
      const mockConfig = {
        isInFallbackMode: () => true,
        getModel: () => DEFAULT_OLLAMA_FLASH_LITE_MODEL,
      } as Config;
      ...
      expect(decision).not.toBeNull();
      expect(decision?.model).toBe(DEFAULT_OLLAMA_FLASH_LITE_MODEL);
      expect(decision?.metadata.source).toBe('fallback');
    });
    ...
    it('should use the flash model if flash is requested', async () => {
      const mockConfig = {
        isInFallbackMode: () => true,
        getModel: () => DEFAULT_OLLAMA_FLASH_MODEL,
      } as Config;
