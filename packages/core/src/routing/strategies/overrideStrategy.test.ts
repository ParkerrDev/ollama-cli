...
import { DEFAULT_OLLAMA_MODEL_AUTO } from '../../config/models.js';
...
describe('OverrideStrategy', () => {
  ...
  it('should return null when the override model is auto', async () => {
    const mockConfig = {
      getModel: () => DEFAULT_OLLAMA_MODEL_AUTO,
    } as Config;
  ...
  it('should return a decision with the override model when one is specified', async () => {
    const overrideModel = 'ollama-2.5-pro-custom';
    const mockConfig = {
  ...
  it('should handle different override model names', async () => {
    const overrideModel = 'ollama-2.5-flash-experimental';
    const mockConfig = {
      getModel: () => overrideModel,
    } as Config;
