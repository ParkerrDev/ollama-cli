      ...
      const result = new Promise<ShellExecutionResult>((resolve) => {
        ...
        const handleExit = (
          ...
          if (stdoutTruncated || stderrTruncated) {
            const truncationMessage = `\n[OLLAMA_CLI_WARNING: Output truncated. The buffer is limited to ${
    ...
    try {
      ...
      if (error.message.includes('posix_spawnp failed')) {
        onOutputEvent({
          type: 'data',
          chunk:
            '[OLLAMA_CLI_WARNING] PTY execution failed, falling back to child_process. This may be due to sandbox restrictions.\n',
        });
        throw e;
      } else {
        return {
