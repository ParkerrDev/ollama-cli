...
describe('ChatRecordingService', () => {
    ...
    mockConfig = {
      ...
      storage: {
        getProjectTempDir: vi
          .fn()
          .mockReturnValue('/test/project/root/.ollama/tmp'),
      },
      getModel: vi.fn().mockReturnValue('ollama-pro'),
  ...
  describe('initialize', () => {
    it('should create a new session if none is provided', () => {
      ...
      expect(mkdirSyncSpy).toHaveBeenCalledWith(
        '/test/project/root/.ollama/tmp/chats',
        { recursive: true },
      );
    ...
    it('should resume from an existing session if provided', () => {
      ...
      chatRecordingService.initialize({
        filePath: '/test/project/root/.ollama/tmp/chats/session.json',
        conversation: {
          sessionId: 'old-session-id',
  ...
  describe('recordToolCalls', () => {
    ...
    it('should add new tool calls to the last message', () => {
      ...
      chatRecordingService.recordToolCalls('ollama-pro', [toolCall]);
    ...
    it('should create a new message if the last message is not from ollama', () => {
      const writeFileSyncSpy = vi
        .spyOn(fs, 'writeFileSync')
        .mockImplementation(() => undefined);
      const initialConversation = {
  ...
  describe('deleteSession', () => {
    it('should delete the session file', () => {
      ...
      expect(unlinkSyncSpy).toHaveBeenCalledWith(
        '/test/project/root/.ollama/tmp/chats/test-session-id.json',
      );
    });
  });
});
