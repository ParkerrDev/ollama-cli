 ...
 *
 * Sessions are stored as JSON files in ~/.ollama/tmp/<project_hash>/chats/
 */
export class ChatRecordingService {
  ...
  recordMessage(message: {
    ...
    try {
      this.updateConversation((conversation) => {
        ...
        if (msg.type === 'ollama') {
          // If it's a new Ollama message then incorporate any queued thoughts.
  ...
  /**
   * Updates the tokens for the last message in the conversation (which should be by Ollama).
   */
  recordMessageTokens(
    ...
    try {
      ...
      this.updateConversation((conversation) => {
        ...
        if (lastMsg && lastMsg.type === 'ollama' && !lastMsg.tokens) {
  ...
  /**
   * Adds tool calls to the last message in the conversation (which should be by Ollama).
  ...
  recordToolCalls(model: string, toolCalls: ToolCallRecord[]): void {
    ...
    try {
      this.updateConversation((conversation) => {
        const lastMsg = this.getLastMessage(conversation);
        // If a tool call was made, but the last message isn't from Ollama, it's because Ollama is
        // calling tools without starting the message with text.  So the user submits a prompt, and
        // Ollama immediately calls a tool (maybe with some thinking first).  In that case, create
        // a new empty Ollama message.
        // Also if there are any queued thoughts, it means this tool call(s) is from a new Ollama
        // message--because it's thought some more since we last, if ever, created a new Ollama
        ...
        if (
          ...
          const newMsg: MessageRecord = {
            ...this.newMessage('ollama' as const, ''),
          ...
          // The last message is an existing Ollama message that we need to update.
