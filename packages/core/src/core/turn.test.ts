...
import { Turn, OllamaEventType } from './turn.js';
import type { GenerateContentResponse, Part, Content } from '@google/genai';
import { reportError } from '../utils/errorReporting.js';
import type { OllamaChat } from './ollamaChat.js';
import { InvalidStreamError, StreamEventType } from './ollamaChat.js';
...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as OllamaChat, 'prompt-id-1');
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerOllamaToolCallRequestEvent;
      expect(event1.type).toBe(OllamaEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerOllamaToolCallRequestEvent;
      expect(event2.type).toBe(OllamaEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: OllamaEventType.Content, value: 'First part' },
    ...
    it('should yield InvalidStream event if sendMessageStream throws InvalidStreamError', async () => {
      ...
      expect(events).toEqual([{ type: OllamaEventType.InvalidStream }]);
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerOllamaErrorEvent;
      expect(errorEvent.type).toBe(OllamaEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerOllamaToolCallRequestEvent;
    ...
    it.each([
      ...
      expect(events).toEqual([
        { type: OllamaEventType.Content, value: contentText },
        {
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: OllamaEventType.Content, value: 'First part' },
        { type: OllamaEventType.Content, value: 'Second part' },
        {
          type: OllamaEventType.Finished,
    ...
    it('should yield citation and finished events when response has citationMetadata', async () => {
      ...
      expect(events).toEqual([
        { type: OllamaEventType.Content, value: 'Some text.' },
        {
    ...
    it('should not yield citation event if there is no finish reason', async () => {
      ...
      expect(events.some((e) => e.type === OllamaEventType.Citation)).toBe(
        false,
      );
    });
    ...
    it('should not crash when cancelled request has malformed error', async () => {
      ...
      expect(events).toEqual([{ type: OllamaEventType.UserCancelled }]);
    ...
    it('should yield a Retry event when it receives one from the chat stream', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: OllamaEventType.Content, value: 'Success' },
      ]);
    });
