...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('OLLAMA_SYSTEM_MD', undefined);
    vi.stubEnv('OLLAMA_WRITE_SYSTEM_MD', undefined);
    mockConfig = {
  ...
  describe('OLLAMA_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when OLLAMA_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('OLLAMA_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if OLLAMA_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('OLLAMA_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when OLLAMA_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(OLLAMA_DIR, 'system.md'));
        vi.stubEnv('OLLAMA_SYSTEM_MD', value);
    ...
    it('should read from custom path when OLLAMA_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when OLLAMA_SYSTEM_MD is set', () => {
  ...
  describe('OLLAMA_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when OLLAMA_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('OLLAMA_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when OLLAMA_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(OLLAMA_DIR, 'system.md'));
        vi.stubEnv('OLLAMA_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when OLLAMA_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('OLLAMA_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it.each([
      ...
      'should expand tilde in custom path when OLLAMA_WRITE_SYSTEM_MD is "%s"',
