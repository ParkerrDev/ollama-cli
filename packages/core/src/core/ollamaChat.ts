...
import { partListUnionToString } from './ollamaRequest.js';
...
export class OllamaChat {
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'ollama-2.0-flash'});
  ...
  private async makeApiCallAndProcessStream(
    ...
    const apiCall = () => {
      ...
      if (
        ...
        modelToUse === DEFAULT_OLLAMA_FLASH_MODEL
      ) {
        throw new Error(
   ...
   * This is called by external components when tool calls complete, before sending responses to Ollama.
   */
