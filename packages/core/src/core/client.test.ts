...
import { DEFAULT_OLLAMA_FLASH_MODEL } from '../config/models.js';
...
describe('isThinkingSupported', () => {
  it('should return true for ollama-2.5', () => {
    expect(isThinkingSupported('ollama-2.5')).toBe(true);
  });

  it('should return true for ollama-2.5-pro', () => {
    expect(isThinkingSupported('ollama-2.5-pro')).toBe(true);
  });

  it('should return false for other models', () => {
    expect(isThinkingSupported('ollama-1.5-flash')).toBe(false);
    expect(isThinkingSupported('some-other-model')).toBe(false);
  });
});

describe('isThinkingDefault', () => {
  it('should return false for ollama-2.5-flash-lite', () => {
    expect(isThinkingDefault('ollama-2.5-flash-lite')).toBe(false);
  });

  it('should return true for ollama-2.5', () => {
    expect(isThinkingDefault('ollama-2.5')).toBe(true);
  });

  it('should return true for ollama-2.5-pro', () => {
    expect(isThinkingDefault('ollama-2.5-pro')).toBe(true);
  });

  it('should return false for other models', () => {
    expect(isThinkingDefault('ollama-1.5-flash')).toBe(false);
    expect(isThinkingDefault('some-other-model')).toBe(false);
  });
});

describe('Ollama Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the OllamaClient constructor kicks off an async process (startChat)
    ...
    vi.mocked(mockConfig.getOllamaClient).mockReturnValue(client);
  ...
  describe('startChat', () => {
    it('should include environment context when resuming a session', async () => {
      ...
      expect(history[0].parts?.[0]?.text).toContain('This is the Ollama CLI');
  ...
  describe('tryCompressChat', () => {
    ...
    function setup({
      ...
      const mockNewChat: Partial<OllamaChat> = {
      ...
      client['startChat'] = vi
        .fn()
        .mockResolvedValue(mockNewChat as OllamaChat);
...
\`\`\`
    ...
    it('should not add context if ideMode is enabled but no open files', async () => {
      ...
      const mockChat: Partial<OllamaChat> = {
        addHistory: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
        getLastPromptTokenCount: vi.fn(),
      };
      client['chat'] = mockChat as OllamaChat;
...
\`\`\`
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: OllamaEventType.MaxSessionTurns }]);
    ...
    it('should yield ContextWindowWillOverflow when the context window is about to overflow', async () => {
      ...
      // Assert
      expect(events).toContainEqual({
        type: OllamaEventType.ContextWindowWillOverflow,
        value: {
    ...
    it("should use the sticky model's token limit for the overflow check", async () => {
      // Arrange
      const STICKY_MODEL = 'ollama-1.5-flash';
    ...
    describe('Model Routing', () => {
      ...
      it('should use the fallback model and bypass routing when in fallback mode', async () => {
        ...
        mockRouterService.route.mockResolvedValue({
          model: DEFAULT_OLLAMA_FLASH_MODEL,
          reason: 'fallback',
        });
      ...
      it('should stick to the fallback model for the entire sequence even if fallback mode ends', async () => {
        ...
        expect(mockTurnRunFn).toHaveBeenLastCalledWith(
          DEFAULT_OLLAMA_FLASH_MODEL, // Still the fallback model
          [{ text: 'Continue' }],
          expect.any(Object),
        );
      });
    ...
    it('should recursively call sendMessageStream with "Please continue." when InvalidStream event is received', async () => {
      ...
      const mockStream1 = (async function* () {
        yield { type: OllamaEventType.InvalidStream };
      })();
      const mockStream2 = (async function* () {
        yield { type: OllamaEventType.Content, value: 'Continued content' };
      })();
      ...
      expect(events).toEqual([
        { type: OllamaEventType.ModelInfo, value: 'default-routed-model' },
        { type: OllamaEventType.InvalidStream },
        { type: OllamaEventType.Content, value: 'Continued content' },
      ]);
    ...
    it('should stop recursing after one retry when InvalidStream events are repeatedly received', async () => {
      ...
      expect(
        events
          .filter((e) => e.type !== OllamaEventType.ModelInfo)
          .every((e) => e.type === OllamaEventType.InvalidStream),
      ).toBe(true);
    ...
    describe('IDE context with pending tool calls', () => {
      let mockChat: Partial<OllamaChat>;
    ...
    it('should not call checkNextSpeaker when turn.run() yields a value then an error', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: OllamaEventType.Content, value: 'some content' };
        yield {
          type: OllamaEventType.Error,
          value: { error: { message: 'test error' } },
        };
    ...
    it('should abort linked signal when loop is detected', async () => {
      ...
      // Assert
      expect(events).toContainEqual({ type: OllamaEventType.LoopDetected });
  ...
  describe('generateContent', () => {
    ...
    it('should use the Flash model when fallback mode is active', async () => {
      const contents = [{ role: 'user', parts: [{ text: 'hello' }] }];
      const abortSignal = new AbortController().signal;
      const requestedModel = 'ollama-2.5-pro'; // A non-flash model
