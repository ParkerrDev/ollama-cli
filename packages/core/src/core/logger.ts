...
export class Logger {
  ...
  async initialize(): Promise<void> {
    ...
    this.ollamaDir = this.storage.getProjectTempDir();
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.ollamaDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.ollamaDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.ollamaDir) {
      debugLogger.error(
