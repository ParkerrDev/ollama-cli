...
import type { ServerOllamaStreamEvent, ChatCompressionInfo } from './turn.js';
...
export function isThinkingSupported(model: string) {
  return model.startsWith('ollama-2.5') || model === DEFAULT_OLLAMA_MODEL_AUTO;
}

export function isThinkingDefault(model: string) {
  if (model.startsWith('ollama-2.5-flash-lite')) {
    return false;
  }
  return model.startsWith('ollama-2.5') || model === DEFAULT_OLLAMA_MODEL_AUTO;
}
...
export class OllamaClient {
  ...
  async startChat(
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing Ollama chat session.',
        history,
        'startChat',
      );
  ...
  private _getEffectiveModelForCurrentTurn(): string {
    ...
    const model: string =
      configModel === DEFAULT_OLLAMA_MODEL_AUTO
        ? DEFAULT_OLLAMA_MODEL
        : configModel;
  ...
  async *sendMessageStream(
    ...
    if (this.currentSequenceModel) {
      ...
      yield { type: OllamaEventType.ModelInfo, value: modelToUse };
    }
    ...
    for await (const event of resultStream) {
      ...
      if (event.type === OllamaEventType.InvalidStream) {
