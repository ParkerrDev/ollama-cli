...
export class AgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  /** Initializes a `OllamaChat` instance for the agent run. */
  private async createChatObject(inputs: AgentInputs): Promise<OllamaChat> {
    ...
    try {
      ...
      await reportError(
        error,
        `Error initializing Ollama chat for agent ${this.definition.name}.`,
        startHistory,
        'startChat',
      );
