...
vi.mock('../core/ollamaChat.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../core/ollamaChat.js')>();
  return {
    ...actual,
    OllamaChat: vi.fn().mockImplementation(() => ({
...
const createTestDefinition = <TOutput extends z.ZodTypeAny>(
  ...
  return {
    ...
    modelConfig: { model: 'ollama-test-model', temp: 0, top_p: 1 },
...
describe('AgentExecutor', () => {
  ...
  beforeEach(async () => {
    ...
    MockedOllamaChat.mockImplementation(
      () =>
        ({
          sendMessageStream: mockSendMessageStream,
          getHistory: vi.fn((_curated?: boolean) => [...mockChatHistory]),
  ...
  describe('create (Initialization and Validation)', () => {
    ...
    it('should correctly apply templates to initialMessages', async () => {
      ...
      const chatConstructorArgs = MockedOllamaChat.mock.calls[0];
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should throw and log if OllamaChat creation fails', async () => {
      const definition = createTestDefinition();
      const initError = new Error('Chat creation failed');
      MockedOllamaChat.mockImplementationOnce(() => {
        throw initError;
      });
