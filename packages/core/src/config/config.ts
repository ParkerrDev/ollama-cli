 ...
 * directly.
 */
export interface OllamaCLIExtension {
  name: string;
  version: string;
  isActive: boolean;
  path: string;
...
export class MCPServerConfig {
  constructor(
    ...
    readonly extension?: OllamaCLIExtension,
    // OAuth configuration
...
export interface ConfigParameters {
  ...
  mcpServers?: Record<string, MCPServerConfig>;
  userMemory?: string;
  ollamaMdFileCount?: number;
  ollamaMdFilePaths?: string[];
...
export class Config {
  ...
  private ollamaMdFileCount: number;
  private ollamaMdFilePaths: string[];
  ...
  constructor(params: ConfigParameters) {
    ...
    this.ollamaMdFileCount = params.ollamaMdFileCount ?? 0;
    this.ollamaMdFilePaths = params.ollamaMdFilePaths ?? [];
    ...
    this.fileFiltering = {
      ...
      respectOllamaIgnore:
        params.fileFiltering?.respectOllamaIgnore ??
    ...
    this.codebaseInvestigatorSettings = {
      ...
      thinkingBudget:
        ...
        DEFAULT_THINKING_MODE,
      model: params.codebaseInvestigatorSettings?.model ?? DEFAULT_OLLAMA_MODEL,
    };
  ...
  async refreshAuth(authMethod: AuthType) {
    ...
    if (this.disableModelRouterForAuth?.includes(authMethod)) {
      ...
      if (this.model === DEFAULT_OLLAMA_MODEL_AUTO) {
  ...
  /**
   * The user configured MCP servers (via ollama settings files).
   *
  ...
  getOllamaMdFileCount(): number {
    return this.ollamaMdFileCount;
  }

  setOllamaMdFileCount(count: number): void {
    this.ollamaMdFileCount = count;
  }

  getOllamaMdFilePaths(): string[] {
    return this.ollamaMdFilePaths;
  }

  setOllamaMdFilePaths(paths: string[]): void {
    this.ollamaMdFilePaths = paths;
  }
  ...
  getFileFilteringRespectOllamaIgnore(): boolean {
    return this.fileFiltering.respectOllamaIgnore;
  }

  getFileFilteringOptions(): FileFilteringOptions {
    return {
      respectGitIgnore: this.fileFiltering.respectGitIgnore,
      respectOllamaIgnore: this.fileFiltering.respectOllamaIgnore,
    };
  }
  ...
  getExtensions(): OllamaCLIExtension[] {
...
export { DEFAULT_OLLAMA_FLASH_MODEL };
