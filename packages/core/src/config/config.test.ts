...
import { setOllamaMdFilename as mockSetOllamaMdFilename } from '../tools/memoryTool.js';
import {
...
vi.mock('../tools/memoryTool', () => ({
  MemoryTool: vi.fn(),
  setOllamaMdFilename: vi.fn(),
  getCurrentOllamaMdFilename: vi.fn(() => 'OLLAMA.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'OLLAMA.md',
  OLLAMA_DIR: '.ollama',
}));
...
describe('Server Config (config.ts)', () => {
  const MODEL = 'ollama-pro';
  const SANDBOX: SandboxConfig = {
    command: 'docker',
    image: 'ollama-cli-sandbox',
  };
  ...
  const EMBEDDING_MODEL = 'ollama-embedding';
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      ...
      expect(OllamaClient).toHaveBeenCalledWith(config);
  ...
  it('Config constructor should call setOllamaMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetOllamaMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setOllamaMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetOllamaMdFilename).not.toHaveBeenCalled();
  });
