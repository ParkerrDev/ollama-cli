...
describe('telemetry/config helpers', () => {
  ...
  describe('resolveTelemetrySettings', () => {
    ...
    it('uses env over settings and argv over env', async () => {
      ...
      const env = {
        OLLAMA_TELEMETRY_ENABLED: '1',
        OLLAMA_TELEMETRY_TARGET: 'gcp',
        OLLAMA_TELEMETRY_OTLP_ENDPOINT: 'http://env:4317',
        OLLAMA_TELEMETRY_OTLP_PROTOCOL: 'http',
        OLLAMA_TELEMETRY_LOG_PROMPTS: 'true',
        OLLAMA_TELEMETRY_OUTFILE: 'env.log',
        OLLAMA_TELEMETRY_USE_COLLECTOR: 'true',
    ...
    it('falls back to OTEL_EXPORTER_OTLP_ENDPOINT when OLLAMA var is missing', async () => {
      const settings = {};
      const env = {
    ...
    it('throws on unknown protocol values', async () => {
      const env = { OLLAMA_TELEMETRY_OTLP_PROTOCOL: 'unknown' } as Record<
        string,
        string
      >;
    ...
    it('throws on unknown target values', async () => {
      const env = { OLLAMA_TELEMETRY_TARGET: 'unknown' } as Record<
        string,
        string
      >;
      await expect(resolveTelemetrySettings({ env })).rejects.toThrow(
