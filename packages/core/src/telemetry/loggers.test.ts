...
import { type OllamaCLIExtension } from '../config/config.js';
import {
  FinishReason,
  type CallableTool,
...
describe('loggers', () => {
  ...
  describe('logToolCall', () => {
    ...
    const mockOllamaClient = new OllamaClient(cfg2);
    const mockConfig = {
      getSessionId: () => 'test-session-id',
      getTargetDir: () => 'target-dir',
      getOllamaClient: () => mockOllamaClient,
  ...
  describe('logModelRouting', () => {
    ...
    it('should log the event to Clearcut and OTEL, and record metrics', () => {
      ...
      expect(mockLogger.emit).toHaveBeenCalledWith({
        body: 'Model routing decision. Model: ollama-pro, Source: default',
        attributes: {
          'session.id': 'test-session-id',
          'user.email': 'test-user@example.com',
          'installation.id': 'test-installation-id',
