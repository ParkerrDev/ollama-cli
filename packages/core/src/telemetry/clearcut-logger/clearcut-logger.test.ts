...
expect.extend({
  ...
  toHaveMetadataValue(
    ...
    const data = metadata.find((m) => m.ollama_cli_key === key)?.value;
  ...
  toHaveMetadataKey(received: LogEventEntry[], key: EventMetadataKey) {
    ...
    const pass = metadata.some((m) => m.ollama_cli_key === key);
...
describe('ClearcutLogger', () => {
  ...
  describe('logChatCompressionEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_COMPRESSION_TOKENS_BEFORE,
        '9001',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_COMPRESSION_TOKENS_AFTER,
        '8000',
      ]);
    });
  });
  ...
  describe('enqueueLogEvent', () => {
    ...
    it('should evict the oldest event when the queue is full', () => {
      ...
      for (let i = 0; i < TEST_ONLY.MAX_EVENTS; i++) {
        logger!.enqueueLogEvent(
          logger!.createLogEvent(EventNames.API_ERROR, [
            {
              ollama_cli_key: EventMetadataKey.OLLAMA_CLI_AI_ADDED_LINES,
              value: `${i}`,
            },
          ]),
        );
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AI_ADDED_LINES,
        '0',
      ]);
  ...
  describe('logModelRoutingEvent', () => {
    it('logs a successful routing event', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_ROUTING_DECISION,
        'ollama-pro',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_ROUTING_DECISION_SOURCE,
        'default-strategy',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_ROUTING_LATENCY_MS,
        '123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_ROUTING_FAILURE,
        'false',
      ]);
    });
    ...
    it('logs a failed routing event with a reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_ROUTING_FAILURE_REASON,
        'Something went wrong',
      ]);
    });
  });
  ...
  describe('logAgentStartEvent', () => {
    it('logs an event with proper fields', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AGENT_ID,
        'agent-123',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AGENT_NAME,
        'TestAgent',
      ]);
    });
  });
  ...
  describe('logAgentFinishEvent', () => {
    it('logs an event with proper fields (success)', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AGENT_DURATION_MS,
        '1000',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AGENT_TURN_COUNT,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AGENT_TERMINATE_REASON,
        'GOAL',
      ]);
    });
  ...
  describe('logToolCallEvent', () => {
    it('logs an event with all diff metadata', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AI_REMOVED_LINES,
        '2',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AI_ADDED_CHARS,
        '3',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_AI_REMOVED_CHARS,
        '4',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_USER_ADDED_LINES,
        '5',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_USER_REMOVED_LINES,
        '6',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_USER_ADDED_CHARS,
        '7',
      ]);
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_USER_REMOVED_CHARS,
        '8',
      ]);
    });
  ...
  describe('logWebFetchFallbackAttemptEvent', () => {
    it('logs an event with the proper name and reason', () => {
      ...
      expect(events[0]).toHaveMetadataValue([
        EventMetadataKey.OLLAMA_CLI_WEB_FETCH_FALLBACK_REASON,
        'private_ip',
      ]);
    });
  });
