    ...
    const mockReaddir = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.ollama/policies'))
        ) {
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.ollama/policies/write.toml'))
        ) {
          return `
[[rule]]
    ...
    const mockReadFile = vi.fn(
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(nodePath.normalize('.ollama/policies/safety.toml'))
        ) {
          return `
[[rule]]
toolName = "write_file"
      ...
      async (
        ...
        if (
          ...
          nodePath
            .normalize(path)
            .includes(
              nodePath.normalize('.ollama/policies/invalid_safety.toml'),
            )
        ) {
          return `
[[rule]]
