...
import { createTmpDir, cleanupTmpDir } from '@google/ollama-cli-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .ollamaignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.ollamaignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .ollamaignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.ollamaignore': 'build/',
      dist: ['ignored-by-git.js'],
      build: ['ignored-by-ollama.js'],
      src: ['not-ignored.js'],
    });
