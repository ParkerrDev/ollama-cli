...
import type { Config, OllamaCLIExtension } from '../config/config.js';
...
export abstract class ExtensionLoader {
   ...
   */
  protected async startExtension(extension: OllamaCLIExtension) {
    if (!this.config) {
    ...
    try {
      ...
      await this.maybeRefreshOllamaTools(extension);
  ...
  private async maybeRefreshMemories(): Promise<void> {
    if (!this.config) {
      throw new Error(
        'Cannot refresh ollama memories prior to calling `start`.',
      );
    }
    if (
  ...
  /**
   * Refreshes the ollama tools list if it is initialized and the extension has
   * any excludeTools settings.
   */
  private async maybeRefreshOllamaTools(
    extension: OllamaCLIExtension,
  ): Promise<void> {
   ...
   * initialized.
   */
  protected async stopExtension(extension: OllamaCLIExtension) {
    if (!this.config) {
  ...
  async restartExtension(extension: OllamaCLIExtension): Promise<void> {
...
export class SimpleExtensionLoader extends ExtensionLoader {
  constructor(
    protected readonly extensions: OllamaCLIExtension[],
  ...
  async loadExtension(extension: OllamaCLIExtension) {
  ...
  async unloadExtension(extension: OllamaCLIExtension) {
