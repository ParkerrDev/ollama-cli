...
import type { OllamaClient } from '../core/client.js';
...
describe('SimpleExtensionLoader', () => {
  ...
  let mockOllamaClientSetTools: MockInstance<
    typeof OllamaClient.prototype.setTools
  >;

  const activeExtension: OllamaCLIExtension = {
    name: 'test-extension',
    isActive: true,
    version: '1.0.0',
  ...
  const inactiveExtension: OllamaCLIExtension = {
    name: 'test-extension',
    isActive: false,
    version: '1.0.0',
    path: '/path/to/extension',
      ...
      (reloadingEnabled) => {
        ...
        it(`should ${reloadingEnabled ? '' : 'not '}reload extension features`, async () => {
          ...
          if (reloadingEnabled) {
            ...
            expect(mockOllamaClientSetTools).toHaveBeenCalledOnce();
          } else {
            expect(mockMcpClientManager.startExtension).not.toHaveBeenCalled();
            expect(mockRefreshServerHierarchicalMemory).not.toHaveBeenCalled();
            expect(mockOllamaClientSetTools).not.toHaveBeenCalledOnce();
          }
          mockRefreshServerHierarchicalMemory.mockClear();
          mockOllamaClientSetTools.mockClear();
...
class TestingSimpleExtensionLoader extends SimpleExtensionLoader {
  override async startExtension(extension: OllamaCLIExtension): Promise<void> {
    await super.startExtension(extension);
  }

  override async stopExtension(extension: OllamaCLIExtension): Promise<void> {
