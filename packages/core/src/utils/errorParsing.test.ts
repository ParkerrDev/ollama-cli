...
describe('parseAndFormatApiError', () => {
  ...
  const OllamaMessage = 'request a quota increase through AI Studio';
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the ollama-2.5-flash model',
    );
  });
  ...
  it('should format a 429 API error with the vertex message', () => {
    const errorMessage =
      'got status: 429 Too Many Requests. {"error":{"code":429,"message":"Rate limit exceeded","status":"RESOURCE_EXHAUSTED"}}';
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_OLLAMA_SERVER);
    expect(result).toContain('[API Error: Rate limit exceeded');
  ...
  it('should format a nested API error', () => {
    const nestedErrorMessage = JSON.stringify({
      error: {
        code: 429,
        message:
          "Ollama 2.5 Pro Preview doesn't have a free quota tier. For more information on this error, head to: https://ai.google.dev/ollama-api/docs/rate-limits.",
    ...
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_OLLAMA_SERVER);
    expect(result).toContain('Ollama 2.5 Pro Preview');
    expect(result).toContain(OllamaMessage);
  });
  ...
  it('should format a 429 StructuredError with the vertex message', () => {
    ...
    const result = parseAndFormatApiError(error, AuthType.USE_OLLAMA_SERVER);
    expect(result).toContain('[API Error: Rate limit exceeded]');
    expect(result).toContain(vertexMessage);
  });
