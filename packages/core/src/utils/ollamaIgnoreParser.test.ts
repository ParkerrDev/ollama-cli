...
import { OllamaIgnoreParser } from './ollamaIgnoreParser.js';
import * as fs from 'node:fs/promises';
import * as path from 'node:path';
import * as os from 'node:os';

describe('OllamaIgnoreParser', () => {
  ...
  beforeEach(async () => {
    projectRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), 'ollamaignore-test-'),
    );
  });
  ...
  describe('when .ollamaignore exists', () => {
    beforeEach(async () => {
    ...
    it('should ignore files specified in .ollamaignore', () => {
      const parser = new OllamaIgnoreParser(projectRoot);
  ...
  describe('when .ollamaignore does not exist', () => {
