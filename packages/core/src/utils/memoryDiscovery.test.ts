...
import { Config, type OllamaCLIExtension } from '../config/config.js';
...
describe('memoryDiscovery', () => {
  ...
  describe('when untrusted', () => {
    ...
    it('loads context from outside the untrusted workspace', async () => {
      ...
      const filepath = path.join(homedir, OLLAMA_DIR, DEFAULT_CONTEXT_FILENAME);
  ...
  it('should load only the global context file if present and others are not (default filename)', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, OLLAMA_DIR, DEFAULT_CONTEXT_FILENAME),
...
--- End of Context from: ${path.relative(cwd, defaultContextFile)} ---`,
  ...
  it('should load only the global custom context file if present and filename is changed', async () => {
    ...
    setOllamaMdFilename(customFilename);

    const customContextFile = await createTestFile(
      path.join(homedir, OLLAMA_DIR, customFilename),
...
--- End of Context from: ${path.join('subdir', customFilename)} ---`,
  ...
  it('should load ORIGINAL_OLLAMA_MD_FILENAME files by upward traversal from CWD to project root', async () => {
    const projectRootOllamaFile = await createTestFile(
      path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      'Project root memory',
    );
    const srcOllamaFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'Src directory memory',
    );
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, projectRootOllamaFile)} ---
Project root memory
--- End of Context from: ${path.relative(cwd, projectRootOllamaFile)} ---

--- Context from: ${path.relative(cwd, srcOllamaFile)} ---
Src directory memory
--- End of Context from: ${path.relative(cwd, srcOllamaFile)} ---`,
      fileCount: 2,
      filePaths: [projectRootOllamaFile, srcOllamaFile],
    });
  });

  it('should load ORIGINAL_OLLAMA_MD_FILENAME files by downward traversal from CWD', async () => {
    const subDirOllamaFile = await createTestFile(
      path.join(cwd, 'subdir', DEFAULT_CONTEXT_FILENAME),
      'Subdir memory',
    );
    const cwdOllamaFile = await createTestFile(
      path.join(cwd, DEFAULT_CONTEXT_FILENAME),
      'CWD memory',
    );
...
--- End of Context from: ${path.join('subdir', DEFAULT_CONTEXT_FILENAME)} ---`,
      fileCount: 2,
      filePaths: [cwdOllamaFile, subDirOllamaFile],
    });
  });

  it('should load and correctly order global, upward, and downward ORIGINAL_OLLAMA_MD_FILENAME files', async () => {
    const defaultContextFile = await createTestFile(
      path.join(homedir, OLLAMA_DIR, DEFAULT_CONTEXT_FILENAME),
      'default context content',
    );
    const rootOllamaFile = await createTestFile(
...
--- Context from: ${path.relative(cwd, rootOllamaFile)} ---
Project parent memory
--- End of Context from: ${path.relative(cwd, rootOllamaFile)} ---

--- Context from: ${path.relative(cwd, projectRootOllamaFile)} ---
Project root memory
--- End of Context from: ${path.relative(cwd, projectRootOllamaFile)} ---

--- Context from: ${path.relative(cwd, cwdOllamaFile)} ---
CWD memory
--- End of Context from: ${path.relative(cwd, cwdOllamaFile)} ---

--- Context from: ${path.relative(cwd, subDirOllamaFile)} ---
Subdir memory
--- End of Context from: ${path.relative(cwd, subDirOllamaFile)} ---`,
      fileCount: 5,
      filePaths: [
        defaultContextFile,
        rootOllamaFile,
        projectRootOllamaFile,
        cwdOllamaFile,
        subDirOllamaFile,
      ],
    });
  ...
  it('should ignore specified directories during downward scan', async () => {
    ...
    const regularSubDirOllamaFile = await createTestFile(
    ...
    expect(result).toEqual({
      memoryContent: `--- Context from: ${path.relative(cwd, regularSubDirOllamaFile)} ---
My code memory
--- End of Context from: ${path.relative(cwd, regularSubDirOllamaFile)} ---`,
      fileCount: 1,
      filePaths: [regularSubDirOllamaFile],
    });
  });
  ...
  it('should load extension context file paths', async () => {
    const extensionFilePath = await createTestFile(
      path.join(testRootDir, 'extensions/ext1/OLLAMA.md'),
...
--- End of Context from: ${path.relative(cwd, includedFile)} ---`,
  ...
  it('should handle multiple directories and files in parallel correctly', async () => {
    // Create multiple test directories with OLLAMA.md files
    const numDirs = 5;
  ...
  describe('loadEnvironmentMemory', () => {
    it('should load extension memory', async () => {
      const extFile = await createTestFile(
        path.join(testRootDir, 'ext', 'OLLAMA.md'),
        'Extension content',
      );
    ...
    it('should keep multiple memory files from the same directory adjacent and in order', async () => {
      ...
      setOllamaMdFilename(['PRIMARY.md', 'SECONDARY.md']);
  ...
  it('refreshServerHierarchicalMemory should refresh memory and update config', async () => {
    ...
    expect(config.getOllamaMdFileCount()).equals(refreshResult.fileCount);
    ...
    expect(config.getOllamaMdFilePaths()).equals(refreshResult.filePaths);
