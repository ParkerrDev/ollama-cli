...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile(OLLAMA_DIR, 'config.yaml');
      await createTestFile(OLLAMA_DIR, 'logs.json');
  ...
  describe('with ollamaignore', () => {
    it('should ignore ollamaignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.ollamaignore'),
        'ignored.txt\nnode_modules/\n.ollama/\n!/.ollama/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectOllamaIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.ollamaignore'),
        'ignored.txt\nnode_modules/\n.ollama/\n!/.ollama/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectOllamaIgnore: false,
          respectGitIgnore: true, // Explicitly disable ollama ignore only
        },
      });
      expect(structure).toContain('ignored.txt');
