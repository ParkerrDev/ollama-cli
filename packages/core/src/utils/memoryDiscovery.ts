...
export async function loadGlobalMemory(
  ...
  const accessChecks = ollamaMdFilenames.map(async (filename) => {
    const globalPath = path.join(userHome, OLLAMA_DIR, filename);
    try {
  ...
  const contents = await readOllamaMdFiles(foundPaths, debugMode, 'tree');
...
/**
 * Traverses upward from startDir to stopDir, finding all OLLAMA.md variants.
 *
 * Files are ordered by directory level (root to leaf), with all filename
 * variants grouped together per directory.
 */
async function findUpwardOllamaFiles(
  startDir: string,
  stopDir: string,
  debugMode: boolean,
): Promise<string[]> {
  ...
  const ollamaMdFilenames = getAllOllamaMdFilenames();
  const globalOllamaDir = path.join(homedir(), OLLAMA_DIR);
...
export async function loadEnvironmentMemory(
  ...
  const traversalPromises = trustedRoots.map(async (root) => {
    ...
    return await findUpwardOllamaFiles(resolvedRoot, resolvedRoot, debugMode);
  });
  ...
  const contents = await readOllamaMdFiles(sortedPaths, debugMode, 'tree');
...
export async function loadServerHierarchicalMemory(
  ...
  if (filePaths.length === 0) {
    if (debugMode)
      logger.debug('No OLLAMA.md files found in hierarchy of the workspace.');
    return { memoryContent: '', fileCount: 0, filePaths: [] };
  }
  const contentsWithPaths = await readOllamaMdFiles(
    filePaths,
    debugMode,
    importFormat,
  );
 ...
 *
 * Returns the result of the call to `loadHierarchicalOllamaMemory`.
 */
export async function refreshServerHierarchicalMemory(config: Config) {
  ...
  config.setOllamaMdFileCount(result.fileCount);
  config.setOllamaMdFilePaths(result.filePaths);
...
export async function loadJitSubdirectoryMemory(
  ...
  const potentialPaths = await findUpwardOllamaFiles(
    resolvedTarget,
    bestRoot,
    debugMode,
  );
  ...
  const contents = await readOllamaMdFiles(newPaths, debugMode, 'tree');
