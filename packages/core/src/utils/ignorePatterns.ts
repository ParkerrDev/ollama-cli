...
import { getCurrentOllamaMdFilename } from '../tools/memoryTool.js';
...
export interface ExcludeOptions {
  ...
  /**
   * Whether to include dynamic patterns like the current Ollama MD filename. Defaults to true.
   */
...
export class FileExclusions {
  ...
  getDefaultExcludePatterns(options: ExcludeOptions = {}): string[] {
    ...
    // Add dynamic patterns (like current Ollama MD filename)
    if (includeDynamicPatterns) {
      patterns.push(`**/${getCurrentOllamaMdFilename()}`);
    }
