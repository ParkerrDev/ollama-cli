...
/**
 * Decoupled LLM request format - stable across Ollama CLI versions
 */
...
/**
 * Decoupled LLM response format - stable across Ollama CLI versions
 */
...
/**
 * Decoupled tool configuration - stable across Ollama CLI versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_OLLAMA_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
