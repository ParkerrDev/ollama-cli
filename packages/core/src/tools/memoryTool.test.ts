...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset OLLAMA_MD_FILENAME to its original value after each test
    setOllamaMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setOllamaMdFilename', () => {
    it('should update currentOllamaMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setOllamaMdFilename(newName);
      expect(getCurrentOllamaMdFilename()).toBe(newName);
    });

    it('should not update currentOllamaMdFilename if the new name is empty or whitespace', () => {
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', OLLAMA_DIR, 'OLLAMA.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(
          path.join('mock', 'home', OLLAMA_DIR),
        );
