...
describe('EditTool', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      getOllamaClient: vi.fn().mockReturnValue(ollamaClient),
      ...
      getIdeMode: () => false,
      // getOllamaConfig: () => ({ apiKey: 'test-api-key' }), // This was not a real Config method
    ...
    it('should use corrected params from ensureCorrectEdit for diff generation', async () => {
      ...
      mockEnsureCorrectEdit.mockImplementationOnce(
        async (_, content, p, client, baseClient) => {
          ...
          expect(client).toBe(ollamaClient);
