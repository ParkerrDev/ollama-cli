...
import { partListUnionToString } from '../core/ollamaRequest.js';
...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it.each<IgnoreFileTestCase>([
      ...
      {
        name: 'should respect .ollamaignore files by default',
        ignoreFile: { name: '.ollamaignore', content: '*.ollamaignored.txt' },
        filesToCreate: ['a.ollamaignored.txt', 'b.notignored.txt'],
        globToolParams: { pattern: '*.txt' },
        expectedCountMessage: 'Found 3 file(s)',
        notExpectedToContain: ['a.ollamaignored.txt'],
      },
      {
      ...
      {
        name: 'should not respect .ollamaignore when respect_ollama_ignore is false',
        ignoreFile: { name: '.ollamaignore', content: '*.ollamaignored.txt' },
        filesToCreate: ['a.ollamaignored.txt'],
        globToolParams: { pattern: '*.txt', respect_ollama_ignore: false },
        expectedCountMessage: 'Found 3 file(s)',
        expectedToContain: ['a.ollamaignored.txt'],
      },
    ])(
      '$name',
      async ({
