...
export class McpClientManager {
   ...
   *    - Updates the Ollama chat configuration to load the new tools.
   */
  async stopExtension(extension: OllamaCLIExtension) {
   ...
   *    - Updates the Ollama chat configuration to load the new tools.
   */
  async startExtension(extension: OllamaCLIExtension) {
  ...
  private async disconnectClient(name: string) {
    ...
    if (existing) {
      try {
        ...
        const ollamaClient = this.cliConfig.getOllamaClient();
   ...
   * ollama settings or command line arguments).
   *
