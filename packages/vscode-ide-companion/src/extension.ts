...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from '@google/ollama-cli-core/src/ide/detect-ide.js';

const CLI_IDE_COMPANION_IDENTIFIER = 'Google.ollama-cli-vscode-ide-companion';
const INFO_MESSAGE_SHOWN_KEY = 'ollamaCliInfoMessageShown';
export const DIFF_SCHEME = 'ollama-diff';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Ollama CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Ollama CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Ollama CLI Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('ollama-cli.runOllamaCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Ollama CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Ollama CLI in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `Ollama CLI (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(ollamaCmd);
      }
    }),
    vscode.commands.registerCommand('ollama-cli.showNotices', async () => {
