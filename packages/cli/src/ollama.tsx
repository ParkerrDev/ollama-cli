...
import type { Config, ResumedSessionData } from '@google/ollama-cli-core';
import {
  sessionId,
  logUserPrompt,
  AuthType,
...
export async function main() {
  ...
  if (
    ...
    settings.merged.security?.auth?.selectedType === AuthType.USE_OLLAMA_SERVER
  ) {
    if (
  ...
  // to run Ollama CLI. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    if (!input) {
      debugLogger.error(
        `No input provided via stdin. Input can be provided by piping data into ollama or using the --prompt option.`,
      );
      process.exit(1);
    }
