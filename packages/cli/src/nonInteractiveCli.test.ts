...
vi.mock('@google/ollama-cli-core', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('@google/ollama-cli-core')>();
...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getOllamaClient: vi.fn().mockReturnValue(mockOllamaClient),
      getToolRegistry: vi.fn().mockReturnValue(mockToolRegistry),
      getMaxSessionTurns: vi.fn().mockReturnValue(10),
      getSessionId: vi.fn().mockReturnValue('test-session-id'),
      getProjectRoot: vi.fn().mockReturnValue('/test/project'),
      storage: {
        getProjectTempDir: vi.fn().mockReturnValue('/test/project/.ollama/tmp'),
      },
  ...
  async function* createStreamFromEvents(
    events: ServerOllamaStreamEvent[],
  ...
  it('should process input and write text output', async () => {
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Hello' },
      { type: OllamaEventType.Content, value: ' World' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockOllamaClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockOllamaClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerOllamaStreamEvent = {
    ...
    const firstCallEvents: ServerOllamaStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Final answer' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockOllamaClient.sendMessageStream
    ...
    expect(mockOllamaClient.sendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockCoreExecuteToolCall).toHaveBeenCalledWith(
      mockConfig,
      expect.objectContaining({ name: 'testTool' }),
      expect.any(AbortSignal),
    );
    expect(mockOllamaClient.sendMessageStream).toHaveBeenNthCalledWith(
      2,
      [{ text: 'Tool response' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should write a single newline between sequential text outputs from the model', async () => {
    ...
    const modelTurn1: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Use mock tool' },
      toolCallEvent,
    ];
    // Turn 2: Model outputs more text, then requests another tool call.
    const modelTurn2: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Use mock tool again' },
      toolCallEvent,
    ];
    // Turn 3: Model outputs a final answer.
    const modelTurn3: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Finished.' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerOllamaStreamEvent[] = [
      {
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockOllamaClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the Ollama client
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Summary complete.' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Hello World' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerOllamaStreamEvent[] = [
      toolCallEvent,
      {
        type: OllamaEventType.Finished,
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Response from command' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Response to unknown' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'Acknowledged' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should allow a normally-excluded tool when --allowed-tools is set', async () => {
    ...
    const firstCallEvents: ServerOllamaStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerOllamaStreamEvent[] = [
      { type: OllamaEventType.Content, value: 'file.txt' },
      {
        type: OllamaEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should display a deprecation warning if hasDeprecatedPromptArg is true', async () => {
    ...
    expect(processStderrSpy).toHaveBeenCalledWith(
      'The --prompt (-p) flag has been deprecated and will be removed in a future version. Please use a positional argument for your prompt. See ollama --help for more information.\n',
    );
