...
import { createTransport, debugLogger } from '@google/ollama-cli-core';
...
vi.mock('@google/ollama-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    Storage: vi.fn().mockImplementation((_cwd: string) => ({
      getGlobalSettingsPath: () => '/tmp/ollama/settings.json',
      getWorkspaceSettingsPath: () => '/tmp/ollama/workspace-settings.json',
      getProjectTempDir: () => '/test/home/.ollama/tmp/mocked_hash',
    })),
    OLLAMA_DIR: '.ollama',
