...
export const AppContainer = (props: AppContainerProps) => {
  ...
  const isOllamaClientInitialized = config.getOllamaClient()?.isInitialized();

  useSessionResume({
    config,
    historyManager,
    refreshStatic,
    isOllamaClientInitialized,
    setQuittingMessages,
    resumedSessionData,
    isAuthenticating,
  });
...
----------------------------------------------------------------
  ...
  const {
    ...
    submitQuery,
    initError,
    pendingHistoryItems: pendingOllamaHistoryItems,
    thought,
  ...
  // Update terminal title with Ollama CLI status and thoughts
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because Ollama CLI is already doing that elsewhere
  }, [
    streamingState,
    thought,
    settings.merged.ui?.showStatusInTitle,
  ...
  const pendingHistoryItems = useMemo(
    () => [...pendingSlashCommandHistoryItems, ...pendingOllamaHistoryItems],
    [pendingSlashCommandHistoryItems, pendingOllamaHistoryItems],
  );

  const [ollamaMdFileCount, setOllamaMdFileCount] = useState<number>(
    config.getOllamaMdFileCount(),
  );
  useEffect(() => {
    const handleMemoryChanged = (result: MemoryChangedPayload) => {
      setOllamaMdFileCount(result.fileCount);
    };
