...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectOllamaIgnore: () => true,
  ...
  describe('ollama-ignore filtering', () => {
    it('should skip ollama-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, '.ollamaignore'),
        'build/output.js',
      );
      const ollamaIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${ollamaIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${ollamaIgnoredFile} is ollama-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nOllama-ignored: ${ollamaIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .ollamaignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, '.ollamaignore'),
      'build/output.js',
    );
  ...
  it('should handle mixed ollama-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, '.ollamaignore'),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${ollamaIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${getRelativePath(validFile)} @${ollamaIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${getRelativePath(validFile)}:\n` },
