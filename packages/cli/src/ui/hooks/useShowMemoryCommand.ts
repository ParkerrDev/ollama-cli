...
import { debugLogger, type Config } from '@google/ollama-cli-core';
...
export function createShowMemoryAction(
  ...
  return async () => {
    ...
    const fileCount = config.getOllamaMdFileCount();
    ...
    if (currentMemory && currentMemory.trim().length > 0) {
      ...
      addMessage({
        ...
        content:
          fileCount > 0
            ? 'Hierarchical memory (OLLAMA.md or other context files) is loaded but content is empty.'
            : 'No hierarchical memory (OLLAMA.md or other context files) is currently loaded.',
        timestamp: new Date(),
      });
    }
  };
