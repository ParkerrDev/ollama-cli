...
import type { ConversationRecord } from '@google/ollama-cli-core';
import type { Part } from '@google/genai';
import { partListUnionToString } from '@google/ollama-cli-core';
import { MessageType, ToolCallStatus } from '../types.js';
/**
 * Converts session/conversation data into UI history and Ollama client history formats.
 */
export function convertSessionToHistoryFormats(
    ...
    if (msg.content && contentString.trim()) {
      ...
      switch (msg.type) {
        case 'user':
          ...
          break;
        default:
          messageType = MessageType.OLLAMA;
          break;
      }
  ...
  // Convert to Ollama client history format
  ...
  for (const msg of messages) {
    ...
    if (msg.type === 'user') {
    ...
    } else if (msg.type === 'ollama') {
      // Handle Ollama messages with potential tool calls
      const hasToolCalls =
      ...
      if (hasToolCalls) {
        ...
        // Regular Ollama message without tool calls
        const contentString = partListUnionToString(msg.content);
        if (msg.content && contentString.trim()) {
          clientHistory.push({
            role: 'model',
