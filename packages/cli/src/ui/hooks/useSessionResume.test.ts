...
describe('useSessionResume', () => {
  ...
  const getDefaultProps = () => ({
    ...
    isOllamaClientInitialized: true,
  ...
  describe('loadHistoryForResume', () => {
    ...
    it('should clear history and add items when loading history', () => {
      ...
      expect(mockOllamaClient.resumeChat).toHaveBeenCalledWith(
        clientHistory,
        resumedData,
      );
    });

    it('should not load history if Ollama client is not initialized', () => {
      const { result } = renderHook(() =>
        useSessionResume({
          ...getDefaultProps(),
          isOllamaClientInitialized: false,
        }),
      );
      ...
      expect(mockOllamaClient.resumeChat).not.toHaveBeenCalled();
    });
    ...
    it('should handle empty history arrays', () => {
      ...
      expect(mockHistoryManager.addItem).not.toHaveBeenCalled();
      expect(mockRefreshStatic).toHaveBeenCalled();
      expect(mockOllamaClient.resumeChat).toHaveBeenCalledWith([], resumedData);
    });
  });
  ...
  describe('automatic resume on mount', () => {
    ...
    it('should not resume when Ollama client is not initialized', () => {
      const conversation: ConversationRecord = {
        sessionId: 'auto-resume-123',
        projectHash: 'project-123',
        startTime: '2025-01-01T00:00:00Z',
    ...
    it('should automatically resume session when resumedSessionData is provided', async () => {
      ...
      expect(mockHistoryManager.addItem).toHaveBeenNthCalledWith(
        2,
        { type: 'ollama', text: 'Welcome back!' },
        1,
      );
      expect(mockOllamaClient.resumeChat).toHaveBeenCalled();
    });
    ...
    it('should convert session messages correctly during auto-resume', async () => {
      ...
      const clientHistory = mockOllamaClient.resumeChat.mock.calls[0][0];
