...
import { useOllamaStream } from './useOllamaStream.js';
...
const MockedOllamaClientClass = vi.hoisted(() =>
...
vi.mock('@google/ollama-cli-core', async (importOriginal) => {
  ...
  return {
    ...actualCoreModule,
    GitService: vi.fn(),
    OllamaClient: MockedOllamaClientClass,
...
// --- Tests for useOllamaStream Hook ---
describe('useOllamaStream', () => {
  ...
  beforeEach(() => {
    ...
    mockAddItem = vi.fn();
    // Define the mock for getOllamaClient
    const mockGetOllamaClient = vi.fn().mockImplementation(() => {
      // MockedOllamaClientClass is defined in the module scope by the previous change.
      // It will use the mockStartChat and mockSendMessageStream that are managed within beforeEach.
      const clientInstance = new MockedOllamaClientClass(mockConfig);
    ...
    // Reset mocks for OllamaClient instance methods (startChat and sendMessageStream)
    // The OllamaClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // OllamaChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.ollama/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = (
    ...
    const client = ollamaClient || mockConfig.getOllamaClient();
          ...
          const newToolCalls = props.toolCalls.map((tc) => {
            ...
            if (
              ...
              return {
                ...
                responseSubmittedToOllama: true, // Mark as "processed"
  ...
  const createMockToolCall = (
    ...
    responseSubmittedToOllama: false,
    confirmationDetails:
  ...
  const renderHookWithDefaults = (
    ...
    return renderHook(() =>
      useOllamaStream(
        new MockedOllamaClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedOllamaClientClass(mockConfig);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Ollama when a command is handled without a tool call', async () => {
    ...
    it('should call Ollama with prompt content when slash command returns a `submit_prompt` action', async () => {
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = renderHook(() =>
        useOllamaStream(
          new MockedOllamaClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerOllamaEventType.Content,
            value: 'This is a truncated response...',
          };
          yield {
            type: ServerOllamaEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerOllamaEventType.ContextWindowWillOverflow,
                value: {
  ...
  it('should process @include commands, adding user turn after processing to prevent race conditions', async () => {
    ...
    const { result } = renderHook(() =>
      useOllamaStream(
        mockConfig.getOllamaClient() as OllamaClient,
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  describe('Thought Reset', () => {
    it('should reset thought to null when starting a new prompt', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerOllamaEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerOllamaEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerOllamaEventType.Error,
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getOllamaClient = vi.fn().mockReturnValue({
        ...new MockedOllamaClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerOllamaEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getOllamaClient = vi.fn().mockReturnValue(mockClient);
