...
interface UseSessionResumeParams {
  ...
  refreshStatic: () => void;
  isOllamaClientInitialized: boolean;
...
export function useSessionResume({
  ...
  const loadHistoryForResume = useCallback(
    (
      ...
      // Give the history to the Ollama client.
      config.getOllamaClient()?.resumeChat(clientHistory, resumedData);
    },
    [config, isOllamaClientInitialized, setQuittingMessages],
  );
