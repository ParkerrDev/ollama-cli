...
vi.mock('./hooks/useOllamaStream.js');
...
import { ShellExecutionService } from '@google/ollama-cli-core';
...
describe('AppContainer State Management', () => {
  ...
  const mockedUseOllamaStream = useOllamaStream as Mock;
  ...
  beforeEach(() => {
    ...
    vi.mock('../utils/windowTitle.js', async () => ({
      computeWindowTitle: vi.fn(
        (folderName: string) =>
          // Default behavior: return "Ollama - {folderName}" unless CLI_TITLE is set
          process.env['CLI_TITLE'] || `Ollama - ${folderName}`,
      ),
    }));
    ...
    mockedUseOllamaStream.mockReturnValue({
      streamingState: 'idle',
      submitQuery: vi.fn(),
      initError: null,
      pendingHistoryItems: [],
  ...
  describe('Session Recording Integration', () => {
    it('provides chat recording service configuration', () => {
      ...
      expect(configWithRecording.getOllamaClient).toBeDefined();
      expect(mockOllamaClient.getChatRecordingService).toBeDefined();
  ...
  describe('Session Resume Flow', () => {
    it('accepts resumed session data', () => {
      ...
      expect(mockOllamaClient.resumeChat).toBeDefined();
  ...
  describe('Quota and Fallback Integration', () => {
    ...
    it('passes a valid proQuotaRequest to UIStateContext when provided by the hook', async () => {
      ...
      const mockRequest = {
        failedModel: 'ollama-pro',
        fallbackModel: 'ollama-flash',
        resolve: vi.fn(),
      };
      mockedUseQuotaAndFallback.mockReturnValue({
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should update terminal title with default text when in Idle state and no thought subject', () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]2;${'Ollama - workspace'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    it('should use CLI_TITLE environment variable when set', () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom Ollama Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]2;${'Custom Ollama Title'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
