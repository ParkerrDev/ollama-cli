...
import { getErrorMessage } from '@google/ollama-cli-core';
    ...
    (async () => {
      ...
      if (!authType) {
        if (process.env['OLLAMA_API_KEY']) {
          onAuthError(
            'Existing API key detected (OLLAMA_API_KEY). Select "Ollama API Key" option to use it.',
          );
        } else {
      ...
      if (
        ...
        onAuthError(
          `Invalid value for OLLAMA_DEFAULT_AUTH_TYPE: "${defaultAuthType}". ` +
