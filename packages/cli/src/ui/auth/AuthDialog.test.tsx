...
import { AuthType, type Config } from '@google/ollama-cli-core';
...
import { clearCachedCredentialFile } from '@google/ollama-cli-core';
...
describe('AuthDialog', () => {
  ...
  it('shows metadata server application default credentials option when OLLAMA_CLI_USE_COMPUTE_ADC env var is true', () => {
  ...
  it('does not show Cloud Shell option when when OLLAMA_CLI_USE_COMPUTE_ADC env var is true', () => {
    process.env['OLLAMA_CLI_USE_COMPUTE_ADC'] = 'true';
    renderWithProviders(<AuthDialog {...props} />);
    const items = mockedRadioButtonSelect.mock.calls[0][0].items;
    expect(items).not.toContainEqual({
  ...
  describe('handleAuthSelect', () => {
    ...
    it('exits process for Login with Google when browser is suppressed', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('Please restart Ollama CLI'),
      );
