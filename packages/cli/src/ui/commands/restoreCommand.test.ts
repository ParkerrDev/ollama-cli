...
describe('restoreCommand', () => {
  ...
  beforeEach(async () => {
    ...
    ollamaTempDir = path.join(testRootDir, OLLAMA_DIR);
    checkpointsDir = path.join(ollamaTempDir, 'checkpoints');
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempDir: vi.fn().mockReturnValue(ollamaTempDir),
      },
  ...
  describe('action', () => {
    it('should return an error if temp dir is not found', async () => {
      ...
      expect(
        ...
        type: 'message',
        messageType: 'error',
        content: 'Could not determine the .ollama directory path.',
      });
    });
