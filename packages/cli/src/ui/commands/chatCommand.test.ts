...
import { AuthType, type OllamaClient } from '@google/ollama-cli-core';
  ...
  describe('resume subcommand', () => {
    ...
    it('should resume a conversation with matching authType', async () => {
      const conversation: Content[] = [
        { role: 'user', parts: [{ text: 'hello ollama' }] },
      ...
      expect(result).toEqual({
        type: 'load_history',
        history: [
          { type: 'user', text: 'hello ollama' },
          { type: 'ollama', text: 'hello world' },
  ...
  describe('share subcommand', () => {
    ...
    beforeEach(() => {
      ...
      vi.spyOn(process, 'cwd').mockReturnValue(
        path.resolve('/usr/local/google/home/myuser/ollama-cli'),
      );
    ...
    it('should default to a json file if no path is provided', async () => {
      ...
      const expectedPath = path.join(
        process.cwd(),
        'ollama-conversation-1234567890.json',
      );
