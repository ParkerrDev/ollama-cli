...
import { decodeTagName } from '@google/ollama-cli-core';
...
const getSavedChatTags = async (
  ...
  const ollamaDir = cfg?.storage?.getProjectTempDir();
  if (!ollamaDir) {
    return [];
  }
  try {
    ...
    const files = await fsPromises.readdir(ollamaDir);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = await config?.getOllamaClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
  ...
  action: async (context, args) => {
    ...
    for (const item of conversation) {
      ...
      if (i > 2 || !hasSystemPrompt) {
        uiHistory.push({
          type: (item.role && rolemap[item.role]) || MessageType.OLLAMA,
          text,
...
const shareCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    if (!filePathArg) {
      filePathArg = `ollama-conversation-${Date.now()}.json`;
    }
    ...
    const chat = await context.services.config?.getOllamaClient()?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
