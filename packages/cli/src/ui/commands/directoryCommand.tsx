...
import { refreshServerHierarchicalMemory } from '@google/ollama-cli-core';
...
async function finishAddingDirectories(
    ...
    addItem(
      {
        ...
        text: `Successfully added OLLAMA.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
      },
      Date.now(),
    );
  } catch (error) {
  ...
  if (added.length > 0) {
    const ollama = config.getOllamaClient();
    if (ollama) {
      await ollama.addDirectoryContext();
    }
    addItem(
      {
        type: MessageType.INFO,
