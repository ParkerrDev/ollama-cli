...
import type { LoadServerHierarchicalMemoryResponse } from '@google/ollama-cli-core/index.js';
...
describe('memoryCommand', () => {
  ...
  describe('/memory show', () => {
    ...
    let mockGetOllamaMdFileCount: Mock;

    beforeEach(() => {
      showCommand = getSubCommand('show');

      mockGetUserMemory = vi.fn();
      mockGetOllamaMdFileCount = vi.fn();

      mockContext = createMockCommandContext({
        services: {
          config: {
            getUserMemory: mockGetUserMemory,
            getOllamaMdFileCount: mockGetOllamaMdFileCount,
    ...
    it('should display a message if memory is empty', async () => {
      ...
      mockGetOllamaMdFileCount.mockReturnValue(0);
    ...
    it('should display the memory content and file count if it exists', async () => {
      ...
      mockGetOllamaMdFileCount.mockReturnValue(1);
  ...
  describe('/memory refresh', () => {
    ...
    let mockSetOllamaMdFileCount: Mock;
    let mockSetOllamaMdFilePaths: Mock;

    beforeEach(() => {
      refreshCommand = getSubCommand('refresh');
      mockSetUserMemory = vi.fn();
      mockSetOllamaMdFileCount = vi.fn();
      mockSetOllamaMdFilePaths = vi.fn();

      const mockConfig = {
        setUserMemory: mockSetUserMemory,
        setOllamaMdFileCount: mockSetOllamaMdFileCount,
        setOllamaMdFilePaths: mockSetOllamaMdFilePaths,
    ...
    it('should display success message when memory is refreshed with content', async () => {
      ...
      const refreshResult: LoadServerHierarchicalMemoryResponse = {
        ...
        fileCount: 2,
        filePaths: ['/path/one/OLLAMA.md', '/path/two/OLLAMA.md'],
      };
    ...
    it('should display an error message if refreshing fails', async () => {
      ...
      expect(mockSetOllamaMdFileCount).not.toHaveBeenCalled();
      expect(mockSetOllamaMdFilePaths).not.toHaveBeenCalled();
  ...
  describe('/memory list', () => {
    ...
    let mockGetOllamaMdfilePaths: Mock;

    beforeEach(() => {
      listCommand = getSubCommand('list');
      mockGetOllamaMdfilePaths = vi.fn();
      mockContext = createMockCommandContext({
        services: {
          config: {
            getOllamaMdFilePaths: mockGetOllamaMdfilePaths,
          },
        },
      });
    });

    it('should display a message if no OLLAMA.md files are found', async () => {
      if (!listCommand.action) throw new Error('Command has no action');

      mockGetOllamaMdfilePaths.mockReturnValue([]);
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: 'No OLLAMA.md files in use.',
        },
        expect.any(Number),
      );
    });
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/OLLAMA.md', '/path/two/OLLAMA.md'];
      mockGetOllamaMdfilePaths.mockReturnValue(filePaths);
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 OLLAMA.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
