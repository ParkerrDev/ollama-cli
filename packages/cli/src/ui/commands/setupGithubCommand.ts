...
import { debugLogger } from '@google/ollama-cli-core';
...
// Add Ollama CLI specific entries to .gitignore file
...
export const setupGithubCommand: SlashCommand = {
  ...
  action: async (
    ...
    const readmeUrl = `https://github.com/google-github-actions/run-ollama-cli/blob/${releaseTag}/README.md#quick-start`;
    ...
    for (const workflow of GITHUB_WORKFLOW_PATHS) {
      downloads.push(
        (async () => {
          const endpoint = `https://raw.githubusercontent.com/google-github-actions/run-ollama-cli/refs/tags/${releaseTag}/examples/workflows/${workflow}`;
    ...
    return {
      ...
      toolArgs: {
        description:
          'Setting up GitHub Actions to triage issues and review PRs with Ollama.',
        command,
      },
    };
  },
