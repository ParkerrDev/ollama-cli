...
import { ToolCallDecision } from '@google/ollama-cli-core';
...
describe('<ModelStatsDisplay />', () => {
  ...
  it('should display stats for multiple models correctly', () => {
    ...
    const output = lastFrame();
    expect(output).toContain('ollama-2.5-pro');
    expect(output).toContain('ollama-2.5-flash');
    expect(output).toMatchSnapshot();
  });
  ...
  it('should display a single model correctly', () => {
    ...
    const output = lastFrame();
    expect(output).toContain('ollama-2.5-pro');
    expect(output).not.toContain('ollama-2.5-flash');
    expect(output).toMatchSnapshot();
  });
});
