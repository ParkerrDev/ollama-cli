...
const renderComponent = (
  ...
  const mockConfig = contextValue
    ? ({
        ...
        getModel: vi.fn(() => DEFAULT_OLLAMA_MODEL_AUTO),
        setModel: vi.fn(),
...
describe('<ModelDialog />', () => {
  ...
  it('passes all model options to DescriptiveRadioButtonSelect', () => {
    ...
    expect(props.items[0].value).toBe(DEFAULT_OLLAMA_MODEL_AUTO);
    expect(props.items[1].value).toBe(DEFAULT_OLLAMA_MODEL);
    expect(props.items[2].value).toBe(DEFAULT_OLLAMA_FLASH_MODEL);
    expect(props.items[3].value).toBe(DEFAULT_OLLAMA_FLASH_LITE_MODEL);
    expect(props.showNumbers).toBe(true);
    unmount();
  });

  it('initializes with the model from ConfigContext', () => {
    const mockGetModel = vi.fn(() => DEFAULT_OLLAMA_FLASH_MODEL);
  ...
  it('initializes with "auto" model if getModel returns undefined', () => {
    ...
    // When getModel returns undefined, preferredModel falls back to DEFAULT_OLLAMA_MODEL_AUTO
  ...
  it('calls config.setModel and onClose when DescriptiveRadioButtonSelect.onSelect is triggered', () => {
    ...
    childOnSelect(DEFAULT_OLLAMA_MODEL);

    // Assert against the default mock provided by renderComponent
    expect(mockConfig?.setModel).toHaveBeenCalledWith(DEFAULT_OLLAMA_MODEL);
  ...
  it('updates initialIndex when config context changes', () => {
    const mockGetModel = vi.fn(() => DEFAULT_OLLAMA_MODEL_AUTO);
    ...
    mockGetModel.mockReturnValue(DEFAULT_OLLAMA_FLASH_LITE_MODEL);
