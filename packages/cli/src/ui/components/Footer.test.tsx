...
import { tildeifyPath, ToolCallDecision } from '@google/ollama-cli-core';
...
describe('<Footer />', () => {
  ...
  describe('sandbox and trust info', () => {
    ...
    it('should display custom sandbox info when SANDBOX env is set', () => {
      vi.stubEnv('SANDBOX', 'ollama-cli-test-sandbox');
      const { lastFrame } = renderWithProviders(<Footer />, {
        width: 120,
...
describe('fallback mode display', () => {
  it('should display Flash model when in fallback mode, not the configured Pro model', () => {
    const { lastFrame } = renderWithProviders(<Footer />, {
      ...
      uiState: {
        sessionStats: mockSessionStats,
        currentModel: 'ollama-2.5-flash', // Fallback active, showing Flash
      },
    });

    // Footer should show the effective model (Flash), not the config model (Pro)
    expect(lastFrame()).toContain('ollama-2.5-flash');
    expect(lastFrame()).not.toContain('ollama-2.5-pro');
  });
  ...
  it('should display Pro model when NOT in fallback mode', () => {
    const { lastFrame } = renderWithProviders(<Footer />, {
      ...
      uiState: {
        sessionStats: mockSessionStats,
        currentModel: 'ollama-2.5-pro', // Normal mode, showing Pro
      },
    });

    expect(lastFrame()).toContain('ollama-2.5-pro');
  });
});
