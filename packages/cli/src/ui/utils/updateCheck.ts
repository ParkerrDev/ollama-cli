...
import { getPackageJson, debugLogger } from '@google/ollama-cli-core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of Ollama CLI is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `Ollama CLI update available! ${currentVersion} → ${latestUpdate}`;
