...
import { isGitRepository, debugLogger } from '@google/ollama-cli-core';

vi.mock('@google/ollama-cli-core', () => ({
...
describe('getInstallationInfo', () => {
  ...
  it('should detect running via npx', () => {
    const npxPath = `/Users/test/.npm/_npx/12345/bin/ollama`;
  ...
  it('should detect running via pnpx', () => {
    const pnpxPath = `/Users/test/.pnpm/_pnpx/12345/bin/ollama`;
  ...
  it('should detect running via bunx', () => {
    const bunxPath = `/Users/test/.bun/install/cache/12345/bin/ollama`;
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/usr/local/bin/ollama';
    process.argv[1] = cliPath;
    mockedRealPathSync.mockReturnValue(cliPath);
    mockedExecSync.mockReturnValue(Buffer.from('ollama-cli')); // Simulate successful command

    const info = getInstallationInfo(projectRoot, false);

    expect(mockedExecSync).toHaveBeenCalledWith(
      'brew list -1 | grep -q "^ollama-cli$"',
      { stdio: 'ignore' },
    );
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/@google/ollama-cli/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g @google/ollama-cli@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/@google/ollama-cli/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add @google/ollama-cli@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/bin/ollama`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g @google/ollama-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect local installation and identify yarn from lockfile', () => {
    const localPath = `${projectRoot}/node_modules/.bin/ollama`;
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    const globalPath = `/usr/local/bin/ollama`;
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g @google/ollama-cli@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
