...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-ollama-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-ollama-cli release:`,
      _error,
    );
    throw new Error(
      `Unable to determine the latest run-ollama-cli release on GitHub.`,
    );
  }
};
