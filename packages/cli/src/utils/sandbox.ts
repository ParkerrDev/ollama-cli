...
import type { Config, SandboxConfig } from '@google/ollama-cli-core';
import {
  coreEvents,
  debugLogger,
  FatalSandboxError,
...
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'ollama-cli-sandbox';
const SANDBOX_NETWORK_NAME = 'ollama-cli-sandbox';
const SANDBOX_PROXY_NAME = 'ollama-cli-sandbox-proxy';
...
function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const projectSandboxBashrc = path.join(OLLAMA_DIR, 'sandbox.bashrc');
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : process.env['DEBUG']
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which ollama)`
        : 'ollama';
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        profileFile = path.join(OLLAMA_DIR, `sandbox-macos-${profile}.sb`);
      }
      ...
      // start and set up proxy if OLLAMA_SANDBOX_PROXY_COMMAND is set
    ...
    // determine full path for ollama-cli to distinguish linked vs installed setting
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under ollama-cli repo
    //
    // note this can only be done with binary linked from ollama-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('ollama-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed ollama binary; ' +
            'run `npm link ./packages/cli` under ollama-cli repo to switch to linked binary.',
        );
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              OLLAMA_SANDBOX: config.command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the ollama-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify ollama-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    // OLLAMA_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['OLLAMA_SANDBOX_PROXY_COMMAND'];
    ...
    if (isIntegrationTest) {
      containerName = `ollama-cli-integration-test-${randomBytes(4).toString(
        'hex',
      )}`;
    ...
    // copy OLLAMA_CLI_TEST_VAR for integration tests
    if (process.env['OLLAMA_CLI_TEST_VAR']) {
      args.push(
        '--env',
        `OLLAMA_CLI_TEST_VAR=${process.env['OLLAMA_CLI_TEST_VAR']}`,
      );
    }
    ...
    for (const envVar of [
      'OLLAMA_CLI_IDE_SERVER_PORT',
      'OLLAMA_CLI_IDE_WORKSPACE_PATH',
      'TERM_PROGRAM',
    ]) {
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(OLLAMA_DIR, 'sandbox.venv');
    ...
    if (process.env['OLLAMA_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
