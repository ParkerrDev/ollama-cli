...
describe('computeWindowTitle', () => {
  ...
  it('should use default Ollama title when CLI_TITLE is not set', () => {
    const result = computeWindowTitle('my-project');
    expect(result).toBe('Ollama - my-project');
  });
  ...
  it('should handle folder names with control characters', () => {
    ...
    expect(result).toBe('Ollama - projectname');
  });
  ...
  it('should handle folder names with special characters', () => {
    ...
    expect(result).toBe('Ollama - project-name_v1.0');
  });
});
