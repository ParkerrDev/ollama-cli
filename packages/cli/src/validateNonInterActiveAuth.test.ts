...
describe('validateNonInterActiveAuth', () => {
  let originalEnvOllamaApiKey: string | undefined;
  ...
  beforeEach(() => {
    originalEnvOllamaApiKey = process.env['OLLAMA_API_KEY'];
  ...
  afterEach(() => {
    if (originalEnvOllamaApiKey !== undefined) {
      process.env['OLLAMA_API_KEY'] = originalEnvOllamaApiKey;
    } else {
      delete process.env['OLLAMA_API_KEY'];
    }
  ...
  it('exits if configuredAuthType does not match enforcedAuthType', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "The enforced authentication type is 'oauth-personal', but the current type is 'ollama-api-key'. Please re-authenticate with the correct type.",
    );
    expect(processExitSpy).toHaveBeenCalledWith(1);
  });
  ...
  describe('JSON output mode', () => {
    ...
    it('prints JSON error when enforced auth mismatches current auth and exits with code 1', async () => {
      ...
      expect(thrown?.message).toBe('process.exit(1) called');
      {
        ...
        expect(payload.error.code).toBe(1);
        expect(payload.error.message).toContain(
          "The enforced authentication type is 'ollama-api-key', but the current type is 'oauth-personal'. Please re-authenticate with the correct type.",
        );
      }
    });
