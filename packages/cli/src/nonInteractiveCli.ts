  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    try {
      ...
      while (true) {
        ...
        const responseStream = ollamaClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (event.type === OllamaEventType.Content) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
                type: JsonStreamEventType.MESSAGE,
                timestamp: new Date().toISOString(),
          ...
          } else if (event.type === OllamaEventType.ToolCallRequest) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === OllamaEventType.LoopDetected) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === OllamaEventType.MaxSessionTurns) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
                type: JsonStreamEventType.ERROR,
                timestamp: new Date().toISOString(),
          ...
          } else if (event.type === OllamaEventType.Error) {
        ...
        if (toolCallRequests.length > 0) {
          ...
          // Record tool calls with full metadata before sending responses to Ollama
          try {
            const currentModel =
              ollamaClient.getCurrentSequenceModel() ?? config.getModel();
            ollamaClient
              .getChat()
