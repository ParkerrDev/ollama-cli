...
import { KeychainTokenStorage } from '@google/ollama-cli-core';
...
describe('extensionSettings', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = os.tmpdir() + path.sep + `ollama-cli-test-home-${Date.now()}`;
    extensionDir = path.join(tempHomeDir, '.ollama', 'extensions', 'test-ext');
