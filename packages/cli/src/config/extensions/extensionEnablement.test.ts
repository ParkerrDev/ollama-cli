...
function createTestDir() {
  const dirPath = fs.mkdtempSync(path.join(os.tmpdir(), 'ollama-test-'));
  return {
    path: dirPath,
...
describe('ExtensionEnablementManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(os.homedir).mockReturnValue(path.join(testDir.path, OLLAMA_DIR));
  ...
  it('should enable a path based on an enable override', () => {
    ...
    manager.enable('ext-test', true, '/Users/chrstn/ollama-cli');

    expect(manager.isEnabled('ext-test', '/Users/chrstn/ollama-cli')).toBe(
      true,
    );
  });
