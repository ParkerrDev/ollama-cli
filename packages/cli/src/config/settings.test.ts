...
import { FatalConfigError, OLLAMA_DIR } from '@google/ollama-cli-core';
import { ExtensionManager } from './extension-manager.js';
import { updateSettingsFilePreservingFormat } from '../utils/commentJson.js';

const MOCK_WORKSPACE_DIR = '/mock/workspace';
// Use the (mocked) OLLAMA_DIR for consistency
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    describe('when OLLAMA_CLI_SYSTEM_SETTINGS_PATH is set', () => {
      const MOCK_ENV_SYSTEM_SETTINGS_PATH = '/mock/env/system/settings.json';

      beforeEach(() => {
        process.env['OLLAMA_CLI_SYSTEM_SETTINGS_PATH'] =
          MOCK_ENV_SYSTEM_SETTINGS_PATH;
      });

      afterEach(() => {
        delete process.env['OLLAMA_CLI_SYSTEM_SETTINGS_PATH'];
      });
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      const ollamaEnvPath = path.resolve(path.join(OLLAMA_DIR, '.env'));
      ...
      (mockFsExistsSync as Mock).mockImplementation((p: fs.PathLike) =>
        [USER_SETTINGS_PATH, ollamaEnvPath].includes(p.toString()),
      );
      ...
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          if (p === USER_SETTINGS_PATH)
            return JSON.stringify(userSettingsContent);
          if (p === ollamaEnvPath) return 'TESTTEST=1234';
          return '{}';
        },
      );
    }
