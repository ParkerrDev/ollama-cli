...
import * as ServerConfig from '@google/ollama-cli-core';
...
vi.mock('@google/ollama-cli-core', async () => {
  const actualServer = await vi.importActual<typeof ServerConfig>(
    '@google/ollama-cli-core',
  );
  return {
    ...actualServer,
    IdeClient: {
...
describe('parseArguments', () => {
  ...
  it('should convert positional query argument to prompt by default', async () => {
    process.argv = ['node', 'script.js', 'Hi Ollama'];
    const argv = await parseArguments({} as Settings);
    expect(argv.query).toBe('Hi Ollama');
    expect(argv.prompt).toBe('Hi Ollama');
    expect(argv.promptInteractive).toBeUndefined();
  });
  ...
  it('should map @path to prompt even when config flags are present', async () => {
    ...
    expect(argv.promptInteractive).toBeUndefined();
    expect(argv.model).toBe('ollama-2.5-pro');
  });

  it('maps unquoted positional @path + arg to prompt (one-shot)', async () => {
    // Simulate: ollama @path ./file.md
    process.argv = ['node', 'script.js', '@path', './file.md'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should handle multiple @path arguments in a single command (one-shot)', async () => {
    // Simulate: ollama @path ./file1.md @path ./file2.md
    process.argv = [
      'node',
      'script.js',
      '@path',
  ...
  it('should handle mixed quoted and unquoted @path arguments (one-shot)', async () => {
    // Simulate: ollama "@path ./file1.md" @path ./file2.md "additional text"
    process.argv = [
      'node',
      'script.js',
      '@path ./file1.md',
...
describe('loadCliConfig', () => {
  ...
  it('should use default fileFilter options when unconfigured', async () => {
    ...
    expect(config.getFileFilteringRespectOllamaIgnore()).toBe(
      DEFAULT_FILE_FILTERING_OPTIONS.respectOllamaIgnore,
    );
  });
});
...
describe('Hierarchical Memory Loading (config.ts) - Placeholder Suite', () => {
  ...
  // To re-enable tests for loadHierarchicalOllamaMemory, ensure that:
  ...
  it.skip('should correctly use mocked homedir for global path', async () => {
    ...
    /*
    const MOCK_OLLAMA_DIR_LOCAL = path.join(
      '/mock/home/user',
      ServerConfig.OLLAMA_DIR,
    );
    const MOCK_GLOBAL_PATH_LOCAL = path.join(
      MOCK_OLLAMA_DIR_LOCAL,
      'OLLAMA.md',
    );
    mockFs({
      [MOCK_GLOBAL_PATH_LOCAL]: { type: 'file', content: 'GlobalContentOnly' },
    });
    const memory = await loadHierarchicalOllamaMemory('/some/other/cwd', false);
...
describe('loadCliConfig model selection', () => {
  ...
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('ollama-2.5-pro');
  });
  ...
  it('always prefers model from argv', async () => {
    process.argv = ['node', 'script.js', '--model', 'ollama-2.5-flash-preview'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {
        model: {
    ...
    expect(config.getModel()).toBe('ollama-2.5-flash-preview');
  });
...
describe('loadCliConfig model selection with model router', () => {
  ...
  it('should use auto model when useModelRouter is true and no model is provided', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_OLLAMA_MODEL_AUTO);
  });
  ...
  it('should use default model when useModelRouter is false and no model is provided', async () => {
    ...
    expect(config.getModel()).toBe(DEFAULT_OLLAMA_MODEL);
  });

  it('should prioritize argv over useModelRouter', async () => {
    process.argv = ['node', 'script.js', '--model', 'ollama-from-argv'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {
        experimental: {
    ...
    expect(config.getModel()).toBe('ollama-from-argv');
  });
  ...
  it('should prioritize settings over useModelRouter', async () => {
    ...
    expect(config.getModel()).toBe('ollama-from-settings');
  });

  it('should prioritize environment variable over useModelRouter', async () => {
    process.argv = ['node', 'script.js'];
    vi.stubEnv('OLLAMA_MODEL', 'ollama-from-env');
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
      {
        experimental: {
    ...
    expect(config.getModel()).toBe('ollama-from-env');
  });
});
...
describe('loadCliConfig interactive', () => {
  ...
  it('should not be interactive if positional prompt words are provided with other flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'ollama-2.5-pro', 'Hello'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig({}, 'test-session', argv);
    expect(config.isInteractive()).toBe(false);
  });
  ...
  it('should be interactive if no positional prompt words are provided with flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'ollama-2.5-pro'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig({}, 'test-session', argv);
    expect(config.isInteractive()).toBe(true);
  });
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectOllamaIgnore',
      getter: (c) => c.getFileFilteringRespectOllamaIgnore(),
      value: true,
    },
    {
      property: 'respectOllamaIgnore',
...
describe('Telemetry configuration via environment variables', () => {
  ...
  it('should prioritize OLLAMA_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should prioritize OLLAMA_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
  ...
  it('should throw when OLLAMA_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
  ...
  it('should prioritize OLLAMA_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('OLLAMA_TELEMETRY_OTLP_ENDPOINT', 'http://ollama.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOtlpEndpoint()).toBe('http://ollama.env.com');
  });

  it('should prioritize OLLAMA_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should prioritize OLLAMA_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should prioritize OLLAMA_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_OUTFILE', '/ollama/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOutfile()).toBe('/ollama/env/telemetry.log');
  });

  it('should prioritize OLLAMA_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should use settings value when OLLAMA_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
  ...
  it('should use settings value when OLLAMA_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const settings: Settings = {
      telemetry: { target: ServerConfig.TelemetryTarget.LOCAL },
  ...
  it("should treat OLLAMA_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig({}, 'test-session', argv);
    expect(config.getTelemetryEnabled()).toBe(true);
  ...
  it("should treat OLLAMA_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
  ...
  it("should treat OLLAMA_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig({}, 'test-session', argv);
  ...
  it("should treat OLLAMA_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('OLLAMA_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments({} as Settings);
    const config = await loadCliConfig(
