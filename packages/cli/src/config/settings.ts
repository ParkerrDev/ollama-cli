...
export function getSystemSettingsPath(): string {
  if (process.env['OLLAMA_CLI_SYSTEM_SETTINGS_PATH']) {
    return process.env['OLLAMA_CLI_SYSTEM_SETTINGS_PATH'];
  }
  if (platform() === 'darwin') {
    return '/Library/Application Support/OllamaCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\ollama-cli\\settings.json';
  } else {
    return '/etc/ollama-cli/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['OLLAMA_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['OLLAMA_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer ollama-specific .env under OLLAMA_DIR
    const ollamaEnvPath = path.join(currentDir, OLLAMA_DIR, '.env');
    if (fs.existsSync(ollamaEnvPath)) {
      return ollamaEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring ollama-specific .env
      const homeOllamaEnvPath = path.join(homedir(), OLLAMA_DIR, '.env');
      if (fs.existsSync(homeOllamaEnvPath)) {
        return homeOllamaEnvPath;
      }
...
export function loadEnvironment(settings: Settings): void {
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(OLLAMA_DIR);
