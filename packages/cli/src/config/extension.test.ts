...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'ollama-cli-test-home-'),
    );
    tempWorkspaceDir = fs.mkdtempSync(
      path.join(tempHomeDir, 'ollama-cli-test-workspace-'),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when OLLAMA.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'OLLAMA.md'),
      ]);
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error and cleanup if ollama-extension.json is missing', async () => {
    ...
    it('should throw an error for invalid JSON in ollama-extension.json', async () => {
    ...
    it('should throw an error for missing name in ollama-extension.json', async () => {
      const sourceExtDir = createExtension({
        extensionsDir: tempHomeDir,
...
This extension will run the following MCP servers:
  ...
  * test-server-2 (remote): https://google.com`,
    ...
    describe('installing from github', () => {
      const gitUrl = 'https://github.com/google/ollama-test-extension.git';
      const extensionName = 'ollama-test-extension';
      ...
      it('should fallback to git clone without consent if no release data is found on first install', async () => {
        ...
        expect(mockRequestConsent).toHaveBeenCalledExactlyOnceWith(
          expect.stringContaining(
            'Installing extension "ollama-test-extension"',
          ),
        );
        expect(mockGit.clone).toHaveBeenCalled();
        const metadataPath = path.join(
  ...
  describe('uninstallExtension', () => {
    ...
    it('should uninstall an extension by its source URL', async () => {
      const gitUrl = 'https://github.com/google/ollama-sql-extension.git';
      const sourceExtDir = createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ollama-sql-extension',
        version: '1.0.0',
        installMetadata: {
          source: gitUrl,
          type: 'git',
      ...
      expect(mockLogExtensionUninstall).toHaveBeenCalled();
      expect(ExtensionUninstallEvent).toHaveBeenCalledWith(
        hashValue('ollama-sql-extension'),
        hashValue('https://github.com/google/ollama-sql-extension'),
        'success',
      );
    });
  ...
  describe('enableExtension', () => {
    ...
    const getActiveExtensions = (): OllamaCLIExtension[] => {
      const extensions = extensionManager.getExtensions();
