...
function getSandboxCommand(
  ...
  if (typeof sandbox === 'string' && sandbox) {
    ...
    throw new FatalSandboxError(
      `Missing sandbox command '${sandbox}' (from OLLAMA_SANDBOX)`,
    );
  }
  ...
  if (sandbox === true) {
    throw new FatalSandboxError(
      'OLLAMA_SANDBOX is true but failed to determine command for sandbox; ' +
...
export async function loadSandboxConfig(
  ...
  const image =
    process.env['OLLAMA_SANDBOX_IMAGE'] ?? packageJson?.config?.sandboxImageUri;
