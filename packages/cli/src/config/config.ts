...
import type { ExtensionEvents } from '@google/ollama-cli-core/src/utils/extensionLoader.js';
...
export async function parseArguments(settings: Settings): Promise<CliArgs> {
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('ollama')
    .usage(
      'Usage: ollama [options] [command]\n\nOllama CLI - Launch an interactive CLI, use -p/--prompt for non-interactive mode',
    )
    ...
    .command('$0 [query..]', 'Launch Ollama CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('resume', {
          ...
          coerce: (value: string): string => {
            // When --resume passed with a value (`ollama --resume 123`): value = "123" (string)
            // When --resume passed without a value (`ollama --resume`): value = "" (string)
...
export async function loadCliConfig(
  ...
  if (argv.sandbox) {
    process.env['OLLAMA_SANDBOX'] = 'true';
  }
  ...
  // directly to the Config constructor in core, and have core handle setOllamaMdFilename.
  // However, loadHierarchicalOllamaMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerOllamaMdFilename(settings.context.fileName);
  } else {
    // Reset to default if not provided in settings.
    setServerOllamaMdFilename(getCurrentOllamaMdFilename());
  }
  ...
  // Call the (now wrapper) loadHierarchicalOllamaMemory which calls the server's version
  ...
  return new Config({
    ...
    ollamaMdFileCount: fileCount,
    ollamaMdFilePaths: filePaths,
    approvalMode,
