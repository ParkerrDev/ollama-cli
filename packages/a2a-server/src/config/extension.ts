...
export const INSTALL_METADATA_FILENAME = '.ollama-extension-install.json';

/**
 * Extension definition as written to disk in ollama-extension.json files.
 * This should *not* be referenced outside of the logic for reading files.
 * If information is required for manipulating extensions (load, unload, update)
 * outside of the loading process that data needs to be stored on the
 * OllamaCLIExtension class defined in Core.
 */
...
export function loadExtensions(workspaceDir: string): OllamaCLIExtension[] {
  const allExtensions = [
    ...loadExtensionsFromDir(workspaceDir),
    ...loadExtensionsFromDir(os.homedir()),
  ];

  const uniqueExtensions: OllamaCLIExtension[] = [];
...
function loadExtensionsFromDir(dir: string): OllamaCLIExtension[] {
  const extensionsDir = path.join(dir, EXTENSIONS_DIRECTORY_NAME);
  if (!fs.existsSync(extensionsDir)) {
    return [];
  }

  const extensions: OllamaCLIExtension[] = [];
...
function loadExtension(extensionDir: string): OllamaCLIExtension | null {
