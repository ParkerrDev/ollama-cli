...
import { safeJsonStringify } from '@google/ollama-cli-core/src/utils/safeJsonStringify.js';
...
describe('extension reloading', () => {
  const sandboxEnv = env['OLLAMA_SANDBOX'];
  ...
  itIf((!sandboxEnv || sandboxEnv === 'false') && platform() !== 'win32')(
    ...
    async () => {
      ...
      const testServerPath = join(rig.testDir!, 'ollama-extension.json');
