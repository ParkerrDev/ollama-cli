...
import { DEFAULT_OLLAMA_MODEL } from '../packages/core/src/config/models.js';
import fs from 'node:fs';
import * as pty from '@lydell/node-pty';
import stripAnsi from 'strip-ansi';
import * as os from 'node:os';
import { OLLAMA_DIR } from '../packages/core/src/utils/paths.js';
...
function getDefaultTimeout() {
  ...
  if (env['OLLAMA_SANDBOX']) return 30000; // 30s in containers
...
export class TestRig {
  ...
  constructor() {
    this.bundlePath = join(__dirname, '..', 'bundle/ollama.js');
    this.testDir = null;
  }
  ...
  setup(
    ...
    const ollamaDir = join(this.testDir, OLLAMA_DIR);
    ...
    const settings = {
      ...
      sandbox:
        env['OLLAMA_SANDBOX'] !== 'false' ? env['OLLAMA_SANDBOX'] : false,
      // Don't show the IDE connection dialog when running from VsCode
      ide: { enabled: false, hasSeenNudge: true },
      ...options.settings, // Allow tests to override/add settings
    };
    writeFileSync(
      join(ollamaDir, 'settings.json'),
  ...
  /**
   * The command and args to use to invoke Ollama CLI. Allows us to switch
   * between using the bundled ollama.js (the default) and using the installed
   * 'ollama' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const isNpmReleaseTest =
      env['INTEGRATION_TEST_USE_INSTALLED_OLLAMA'] === 'true';
    const command = isNpmReleaseTest ? 'ollama' : 'node';
    ...
    return poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `ollama_cli.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'ollama_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'ollama_cli.tool_call'
      ) {
  ...
  readLastApiRequest(): ParsedLog | null {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === 'ollama_cli.api_request',
    );
  ...
  async waitForMetric(metricName: string, timeout?: number) {
    ...
    const fullName = metricName.startsWith('ollama_cli.')
      ? metricName
      : `ollama_cli.${metricName}`;
  ...
  readMetric(metricName: string): Record<string, unknown> | null {
    ...
    for (const logData of logs) {
      if (logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `ollama_cli.${metricName}`) {
              return metric;
            }
          }
        }
