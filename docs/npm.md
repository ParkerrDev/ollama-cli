...
This monorepo contains two main packages: `@google/ollama-cli` and
`@google/ollama-cli-core`.

## `@google/ollama-cli`

This is the main package for the Ollama CLI. It is responsible for the user
...
`@google/ollama-cli-core`. This means that whether a user installs the package
with `npm install -g @google/ollama-cli` or runs it directly with
`npx @google/ollama-cli`, they are using this single, self-contained executable.

## `@google/ollama-cli-core`

This package contains the core logic for interacting with the Ollama API. It is
...
- **Simplified Script Execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @google/ollama-cli`.
