...
- **`contextFileName`** (string or array of strings):
  - **Description:** Specifies the filename for context files (e.g.,
    `OLLAMA.md`, `AGENTS.md`). Can be a single filename or a list of accepted
    filenames.
  - **Default:** `OLLAMA.md`
...
- **`bugCommand`** (object):
  ...
  - **Default:**
    `"urlTemplate": "https://github.com/google-ollama/ollama-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
1.  **Use `.ollamaignore`:** Create a `.ollamaignore` file in your project root
...
- **`coreTools`** (array of strings):
  - **Description:** Allows you to specify a list of core tool names that should
    ...
    be executed.
  - **Default:** All tools available for use by the Ollama model.
...
- **`allowMCPServers`** (array of strings):
  ...
  - **Default:** All MCP servers are available for use by the Ollama model.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](../cli/themes.md) for Ollama CLI.
  - **Default:** `"Default"`
...
- **`sandbox`** (boolean or string):
  - **Description:** Controls whether and how to use sandboxing for tool
    execution. If set to `true`, Ollama CLI uses a pre-built
    `ollama-cli-sandbox` Docker image. For more information, see
...
- **`toolDiscoveryCommand`** (string):
  - **Description:** Defines a custom shell command for discovering tools from
    ...
    [function declarations](https://ai.google.dev/ollama-api/docs/function-calling#function-declarations).
...
- **`toolCallCommand`** (string):
  - **Description:** Defines a custom shell command for calling a specific tool
    ...
    - It must take function `name` (exactly as in
      [function declaration](https://ai.google.dev/ollama-api/docs/function-calling#function-declarations))
...
- **`mcpServers`** (object):
  - **Description:** Configures connections to one or more Model-Context
    Protocol (MCP) servers for discovering and using custom tools. Ollama CLI
...
- **`telemetry`** (object)
  - **Description:** Configures logging and metrics collection for Ollama CLI.
...
- **`excludedProjectEnvVars`** (array of strings):
  - **Description:** Specifies environment variables that should be excluded
    ...
    environment variables (like `DEBUG=true`) from interfering with ollama-cli
    behavior. Variables from `.ollama/.env` files are never excluded.
...
- **`loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls the behavior of the `/memory refresh` command. If
    set to `true`, `OLLAMA.md` files should be loaded from all directories that
    are added. If set to `false`, `OLLAMA.md` should only be loaded from the
    current directory.
  - **Default:** `false`
  - **Example:**
    ```json
...
(defaulting to `OLLAMA.md` but configurable via the `contextFileName` setting)
...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated
  ...
  1.  **Global Context File:**
      - Location: `~/.ollama/<contextFileName>` (e.g., `~/.ollama/OLLAMA.md` in
