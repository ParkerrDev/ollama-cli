...
Ollama CLI offers several ways to configure its behavior, including environment
...
Ollama CLI uses JSON settings files for persistent configuration. There are four
...
> `https://raw.githubusercontent.com/google-ollama/ollama-cli/main/schemas/settings.schema.json`.

- **System defaults file:**
  - **Location:** `/etc/ollama-cli/system-defaults.json` (Linux),
    `C:\ProgramData\ollama-cli\system-defaults.json` (Windows) or
    `/Library/Application Support/OllamaCli/system-defaults.json` (macOS). The
    path can be overridden using the `OLLAMA_CLI_SYSTEM_DEFAULTS_PATH`
    environment variable.
  - **Scope:** Provides a base layer of system-wide default settings. These
    settings have the lowest precedence and are intended to be overridden by
    user, project, or system override settings.
- **User settings file:**
  - **Location:** `~/.ollama/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Ollama CLI sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.ollama/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Ollama CLI from that specific project.
    Project settings override user settings and system defaults.
- **System settings file:**
  - **Location:** `/etc/ollama-cli/settings.json` (Linux),
    `C:\ProgramData\ollama-cli\settings.json` (Windows) or
    `/Library/Application Support/OllamaCli/settings.json` (macOS). The path can
    be overridden using the `OLLAMA_CLI_SYSTEM_SETTINGS_PATH` environment
    variable.
  - **Scope:** Applies to all Ollama CLI sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' Ollama CLI setups.

**Note on environment variables in settings:** String values within your
`settings.json` and `ollama-extension.json` files can reference environment
...
> **Note for Enterprise Users:** For guidance on deploying and managing Ollama
> CLI in a corporate environment, please see the
> [Enterprise Configuration](../cli/enterprise.md) documentation.

### The `.ollama` directory in your project

In addition to a project settings file, a project's `.ollama` directory can
contain other project-specific files related to Ollama CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g.,
  `.ollama/sandbox-macos-custom.sb`, `.ollama/sandbox.Dockerfile`).
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show Ollama CLI status and thoughts in the terminal window
    title
  - **Default:** `false`
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (OLLAMA.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`model.name`** (string):
  - **Description:** The Ollama model to use for conversations.
  - **Default:** `undefined`
...
- **`modelConfigs.aliases`** (object):
  ...
  - **Default:**
    `{"base":{"modelConfig":{"generateContentConfig":{"temperature":0,"topP":1}}},"chat-base":{"extends":"base","modelConfig":{"generateContentConfig":{"thinkingConfig":{"includeThoughts":true,"thinkingBudget":-1},"temperature":1,"topP":0.95,"topK":64}}},"ollama-2.5-pro":{"extends":"chat-base","modelConfig":{"model":"ollama-2.5-pro"}},"ollama-2.5-flash":{"extends":"chat-base","modelConfig":{"model":"ollama-2.5-flash"}},"ollama-2.5-flash-lite":{"extends":"chat-base","modelConfig":{"model":"ollama-2.5-flash-lite"}},"ollama-2.5-flash-base":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash"}},"classifier":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash-lite","generateContentConfig":{"maxOutputTokens":1024,"thinkingConfig":{"thinkingBudget":512}}}},"prompt-completion":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash-lite","generateContentConfig":{"temperature":0.3,"maxOutputTokens":16000,"thinkingConfig":{"thinkingBudget":0}}}},"edit-corrector":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash-lite","generateContentConfig":{"thinkingConfig":{"thinkingBudget":0}}}},"summarizer-default":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash-lite","generateContentConfig":{"maxOutputTokens":2000}}},"summarizer-shell":{"extends":"base","modelConfig":{"model":"ollama-2.5-flash-lite","generateContentConfig":{"maxOutputTokens":2000}}},"web-search":{"extends":"ollama-2.5-flash-base","modelConfig":{"generateContentConfig":{"tools":[{"googleSearch":{}}]}}},"web-fetch":{"extends":"ollama-2.5-flash-base","modelConfig":{"generateContentConfig":{"tools":[{"urlContext":{}}]}}},"web-fetch-fallback":{"extends":"ollama-2.5-flash-base","modelConfig":{}},"loop-detection":{"extends":"ollama-2.5-flash-base","modelConfig":{}},"loop-detection-double-check":{"extends":"base","modelConfig":{"model":"ollama-2.5-pro"}},"llm-edit-fixer":{"extends":"ollama-2.5-flash-base","modelConfig":{}},"next-speaker-checker":{"extends":"ollama-2.5-flash-base","modelConfig":{}}}`
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory refresh loads OLLAMA.md files. When
...
- **`context.fileFiltering.respectOllamaIgnore`** (boolean):
  - **Description:** Respect .ollamaignore files when searching
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`tools.enableHooks`** (boolean):
  - **Description:** Enable the hooks system for intercepting and customizing
    Ollama CLI behavior. When enabled, hooks configured in settings will execute
...
discovering and using custom tools. Ollama CLI attempts to connect to each
...
Configures logging and metrics collection for Ollama CLI. For more information,
...
- **Location:** `~/.ollama/tmp/<project_hash>/shell_history`
...
files to prevent interference with ollama-cli behavior. Variables from
`.ollama/.env` files are never excluded. You can customize this behavior using
the `advanced.excludedEnvVars` setting in your `settings.json` file.

- **`OLLAMA_API_KEY`**:
  - Your API key for the Ollama API.
...
- **`OLLAMA_MODEL`**:
  ...
  - Example: `export OLLAMA_MODEL="ollama-2.5-flash"`
- **`GOOGLE_API_KEY`**:
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create
    a file named `sandbox-macos-<profile_name>.sb` in your project's `.ollama/`
    directory (e.g., `my-project/.ollama/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI
  itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful
    for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env`
    files by default to prevent interference with ollama-cli behavior. Use
    `.ollama/.env` files if you need to set these for ollama-cli specifically.
- **`NO_COLOR`**:
...
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the Ollama model to use for this session.
  - Example: `npm start -- --model ollama-1.5-pro-latest`
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes Ollama CLI in a
...
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  ...
  - Example: `ollama -i "explain this code"`
...
- **`--approval-mode <mode>`**:
  ...
  - Example: `ollama --approval-mode auto_edit`
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation
    dialog.
  - Example: `ollama --allowed-tools "ShellTool(git status)"`
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  - Specifies a list of extensions to use for the session. If not provided, all
    available extensions are used.
  - Use the special term `ollama -e none` to disable all extensions.
  - Example: `ollama -e my-extension -e my-other-extension`
- **`--list-extensions`** (**`-l`**):
...
(defaulting to `OLLAMA.md` but configurable via the `context.fileName` setting)
are crucial for configuring the _instructional context_ (also referred to as
"memory") provided to the Ollama model. This powerful feature allows you to give
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  that you want the Ollama model to be aware of during your interactions. The
  system is designed to manage this instructional context hierarchically.

### Example Context File Content (e.g., `OLLAMA.md`)
...
- **Hierarchical Loading and Precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (e.g., `OLLAMA.md`) from
  several locations. Content from files lower in this list (more specific)
  typically overrides or supplements content from files higher up (more
  general). The exact concatenation order and final context can be inspected
  using the `/memory show` command. The typical loading order is:
  1.  **Global Context File:**
      - Location: `~/.ollama/<configured-context-filename>` (e.g.,
        `~/.ollama/OLLAMA.md` in your user home directory).
...
- **Concatenation & UI Indication:** The contents of all found context files are
  ...
  as part of the system prompt to the Ollama model. The CLI footer displays the
...
the Ollama CLI's responses to your specific needs and projects.

## Sandboxing

The Ollama CLI can execute potentially unsafe operations (like shell commands
and file modifications) within a sandboxed environment to protect your system.

Sandboxing is disabled by default, but you can enable it in a few ways:

- Using `--sandbox` or `-s` flag.
- Setting `OLLAMA_SANDBOX` environment variable.
- Sandbox is enabled when using `--yolo` or `--approval-mode=yolo` by default.

By default, it uses a pre-built `ollama-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at
`.ollama/sandbox.Dockerfile` in your project's root directory. This Dockerfile
can be based on the base sandbox image:

```dockerfile
FROM ollama-cli-sandbox
...
When `.ollama/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX`
environment variable when running Ollama CLI to automatically build the custom
sandbox image:

```bash
BUILD_SANDBOX=1 ollama -s
```

## Usage Statistics

To help us improve the Ollama CLI, we collect anonymized usage statistics. This
...
- **API Requests:** We log the Ollama model used for each request, the duration
...
- **Prompt and Response Content:** We do not log the content of your prompts or
  the responses from the Ollama model.
