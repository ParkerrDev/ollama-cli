# Ollama CLI Authentication Setup

Ollama CLI requires authentication using Google's services. Before using Ollama
...
If you are running the Ollama CLI within a Google Cloud Shell environment,
...
support automatic authentication. In these environments, Ollama CLI can
...
When you run Ollama CLI through the command-line, Ollama CLI will provide the
following options:

```bash
> 1. Login with Google
> 2. Use Ollama API key
> 3. Vertex AI
```
...
If you are running Ollama CLI on your local machine, the simplest method is
logging in with your Google account.

> **Important:** Use this method if you are a **Google AI Pro** or **Google AI
> Ultra** subscriber.

1. Select **Login with Google**. Ollama CLI will open a login prompt using your
   web browser.
...
- Using a Ollama Code Assist license from the Google Developer Program.
- Using a license from a Ollama Code Assist subscription.
- Using the product outside the
  [supported regions](https://developers.google.com/ollama-code-assist/resources/available-locations)
...
2.  [Enable the Ollama for Cloud API](https://cloud.google.com/ollama/docs/discover/set-up-ollama#enable-api).
3.  [Configure necessary IAM access permissions](https://cloud.google.com/ollama/docs/discover/set-up-ollama#grant-iam).
...
2.  Set the `OLLAMA_API_KEY` environment variable:
    ...
    ```bash
    # Replace YOUR_OLLAMA_API_KEY with the key from AI Studio
...
> **Warning:** Treat API keys, especially for services like Ollama, as sensitive
...
> **Note:** If you have previously set `GOOGLE_API_KEY` or `OLLAMA_API_KEY`, you
> must unset them to use ADC:

```bash
unset GOOGLE_API_KEY OLLAMA_API_KEY
```
...
2.  **Use a `.env` file:** Create a `.ollama/.env` file in your project
    directory or home directory. Ollama CLI automatically loads variables from
    the first `.env` file it finds, searching up from the current directory,
    then in `~/.ollama/.env` or `~/.env`. `.ollama/.env` is recommended.
    ...
    # Add other variables like OLLAMA_API_KEY as needed
    EOF
    ```
...
1.  **Ollama API Key:** Set `OLLAMA_API_KEY`.
2.  **Vertex AI:**
...
- [Ollama CLI: Quotas and Pricing](../quota-and-pricing.md).
- [Ollama CLI: Terms of Service and Privacy Notice](../tos-privacy.md).
