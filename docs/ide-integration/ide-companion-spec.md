# Ollama CLI Companion Plugin: Interface Specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
Ollama CLI's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.ollama-cli-vscode-ide-companion)).
...
Ollama CLI and the IDE plugin communicate through a local communication channel.
...
For Ollama CLI to connect, it needs to discover which IDE instance it's running
...
- **File Location:** The file must be created in a specific directory:
  `os.tmpdir()/ollama/ide/`. Your plugin must create this directory if it
  doesn't exist.
- **File Naming Convention:** The filename is critical and **MUST** follow the
  pattern: `ollama-ide-server-${PID}-${PORT}.json`
...
- **Tie-Breaking with Environment Variables (Recommended):** For the most
  ...
  set the `OLLAMA_CLI_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `OLLAMA_CLI_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
