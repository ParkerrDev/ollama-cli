...
Ollama CLI can integrate with your IDE to provide a more seamless and
...
- **Native Diffing:** When Ollama suggests code modifications, you can view the
...
- **VS Code Commands:** You can access Ollama CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Ollama CLI: Run`: Starts a new Ollama CLI session in the integrated
    terminal.
  - `Ollama CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `Ollama CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Ollama CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Ollama CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Ollama CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.ollama-cli-vscode-ide-companion).
- **For VS Code Forks:** To support forks of VS Code, the extension is also
  published on the
  [Open VSX Registry](https://open-vsx.org/extension/google/ollama-cli-vscode-ide-companion).
  Follow your editor's instructions for installing extensions from this
  registry.

> NOTE: The "Ollama CLI Companion" extension may appear towards the bottom of
...
When enabled, Ollama CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask Ollama to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **Ollama CLI: Accept Diff**.
...
- Open the Command Palette and run **Ollama CLI: Close Diff Editor**.
...
If you are using Ollama CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker Container:** If you run Ollama CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Ollama CLI could not find the necessary environment variables
    (`OLLAMA_CLI_IDE_WORKSPACE_PATH` or `OLLAMA_CLI_IDE_SERVER_PORT`) to connect
    to the IDE. This usually means the IDE companion extension is not running or
    did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **Ollama CLI Companion** extension in
...
- **Message:**
  `🔴 Disconnected: Directory mismatch. Ollama CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Ollama CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Ollama CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Ollama CLI from the integrated terminal of a supported
    IDE, like VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Ollama CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Ollama CLI
    Companion", and
