...
To enable dev traces, set the `OLLAMA_DEV_TRACING=true` environment variable
when running Ollama CLI.
...
2.  **Run Ollama CLI with Dev Tracing:**

    In a separate terminal, run your Ollama CLI command with the
    `OLLAMA_DEV_TRACING` environment variable:

    ```bash
    OLLAMA_DEV_TRACING=true ollama
    ```
...
```typescript
import { runInDevTraceSpan } from '@google/ollama-cli-core';
...
await runInDevTraceSpan({ name: 'my-custom-span' }, async ({ metadata }) => {
  ...
  metadata.attributes['gen_ai.request.model'] = 'ollama-4.0-mega';
