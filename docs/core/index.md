...
Ollama CLI's core package (`packages/core`) is the backend portion of Ollama
CLI, handling communication with the Ollama API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of Ollama CLI, see the
...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  OLLAMA.md import feature using @file.md syntax.
- **[Policy Engine](./policy-engine.md):** Use the Policy Engine for
  fine-grained control over tool execution.

## Role of the core

While the `packages/cli` portion of Ollama CLI provides the user interface,
`packages/core` is responsible for:

- **Ollama API interaction:** Securely communicating with the Google Ollama API,
  sending user prompts, and receiving model responses.
- **Prompt engineering:** Constructing effective prompts for the Ollama model,
  potentially incorporating conversation history, tool definitions, and
  instructional context from `OLLAMA.md` files.
- **Tool management & orchestration:**
  - Registering available tools (e.g., file system tools, shell command
    execution).
  - Interpreting tool use requests from the Ollama model.
  - Executing the requested tools with the provided arguments.
  - Returning tool execution results to the Ollama model for further processing.
...
- **API key management:** It handles the `OLLAMA_API_KEY` and ensures it's used
  securely when communicating with the Ollama API.
...
To ensure that long conversations don't exceed the token limits of the Ollama
...
[Google AI documentation](https://ai.google.dev/ollama-api/docs/models).

## Model fallback

Ollama CLI includes a model fallback mechanism to ensure that you can continue
...
`OLLAMA.md` files that provide context to the model. It searches for these files
...
`refresh` the content of loaded `OLLAMA.md` files.

## Citations

When Ollama finds it is reciting text from a source it appends the citation to
