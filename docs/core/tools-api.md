# Ollama CLI Core: Tools API

The Ollama CLI core (`packages/core`) features a robust system for defining,
registering, and executing tools. These tools extend the capabilities of the
Ollama model, allowing it to interact with the local environment, fetch web
...
- **Tool (`tools.ts`):** An interface and base class (`BaseTool`) that defines
  ...
  - `name`: A unique internal name (used in API calls to Ollama).
  ...
  - `parameterSchema`: A JSON schema defining the parameters that the tool
    accepts. This is crucial for the Ollama model to understand how to call the
    tool correctly.
...
- **Tool Registry (`tool-registry.ts`):** A class (`ToolRegistry`) responsible
  ...
  - **Providing Schemas:** Exposing the `FunctionDeclaration` schemas of all
    registered tools to the Ollama model, so it knows what tools are available
    and how to use them.
...
1.  **Model Request:** The Ollama model, based on the user's prompt and the
...
8.  **Response to Model:** The `llmContent` from the `ToolResult` is packaged as
    a `FunctionResponse` and sent back to the Ollama model so it can continue
...
- **Command-based Discovery:** Advanced users or project administrators can
  ...
  by the Ollama CLI core, should output a JSON array of `FunctionDeclaration`
...
- **MCP Server(s):** For more complex scenarios, one or more MCP servers can be
  ...
  Ollama CLI core can then discover and use tools exposed by these servers. As
...
This tool system provides a flexible and powerful way to augment the Ollama
model's capabilities, making the Ollama CLI a versatile assistant for a wide
range of tasks.
