...
The Ollama CLI includes a Checkpointing feature that automatically saves a
...
1.  **A Git Snapshot:** A commit is made in a special, shadow Git repository
    located in your home directory (`~/.ollama/history/<project_hash>`). This
...
`~/.ollama/tmp/<project_hash>/checkpoints`.
