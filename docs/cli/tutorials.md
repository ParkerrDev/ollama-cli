...
This page contains tutorials for interacting with Ollama CLI.
...
[`.ollama/settings.json` file](../get-started/configuration.md). Within the
...
Ollama CLI uses this value in the `mcpServers` configuration that you defined in
the `settings.json` file.

#### Launch Ollama CLI and verify the connection

When you launch Ollama CLI, it automatically reads your configuration and
launches the GitHub MCP server in the background. You can then use natural
language prompts to ask Ollama CLI to perform GitHub actions. For example:
