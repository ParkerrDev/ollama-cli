...
which projects can use the full capabilities of the Ollama CLI. It prevents
...
Once the feature is enabled, the first time you run the Ollama CLI from a
...
Your choice is saved in a central file (`~/.ollama/trustedFolders.json`), so you
will only be asked once per folder.

## Why Trust Matters: The Impact of an Untrusted Workspace

When a folder is **untrusted**, the Ollama CLI runs in a restricted "safe mode"
...
1.  **Workspace Settings are Ignored**: The CLI will **not** load the
    `.ollama/settings.json` file from the project. This prevents the loading of
...
Granting trust to a folder unlocks the full functionality of the Ollama CLI for
that workspace.
...
- **View All Trust Rules**: To see a complete list of all your trusted and
  ...
  `~/.ollama/trustedFolders.json` file in your home directory.
...
2.  **Local Trust File**: If the IDE is not connected, the CLI checks the
    central `~/.ollama/trustedFolders.json` file.
