...
Ollama CLI supports several built-in commands to help you manage your session,
...
- **`/bug`**
  - **Description:** File an issue about Ollama CLI. By default, the issue is
    filed within the GitHub repository for Ollama CLI. The string you enter
    ...
    your `.ollama/settings.json` files.
  ...
  - **Sub-commands:**
    - **`save`**
      ...
      - **Details on Checkpoint Location:** The default locations for saved chat
        checkpoints are:
        - Linux/macOS: `~/.ollama/tmp/<project_hash>/`
        - Windows: `C:\Users\<YourUsername>\.ollama\tmp\<project_hash>\`
    ...
    - **`resume`**
      ...
      - **Note:** You can only resume chats that were saved within the current
        ...
        Ollama CLI from that project's directory.
    - **`list`**
...
- **`/copy`**
  - **Description:** Copies the last output produced by Ollama CLI to your
...
- **`/extensions`**
  - **Description:** Lists all active extensions in the current Ollama CLI
    session. See [Ollama CLI Extensions](../extensions/index.md).

- **`/help`** (or **`/?`**)
  - **Description:** Display help information about Ollama CLI, including
...
- [**`/model`**](./model.md)
  - **Description:** Opens a dialog to choose your Ollama model.

- **`/memory`**
  - **Description:** Manage the AI's instructional context (hierarchical memory
    loaded from `OLLAMA.md` files).
  - **Sub-commands:**
    - **`add`**:
    ...
    - **`show`**:
      - **Description:** Display the full, concatenated content of the current
        hierarchical memory that has been loaded from all `OLLAMA.md` files.
        This lets you inspect the instructional context being provided to the
        Ollama model.
    - **`refresh`**:
      - **Description:** Reload the hierarchical instructional memory from all
        `OLLAMA.md` files found in the configured locations (global,
        project/ancestors, and sub-directories). This command updates the model
        with the latest `OLLAMA.md` content.
    - **`list`**:
      - **Description:** Lists the paths of the OLLAMA.md files in use for
        hierarchical memory.
    - **Note:** For more details on how `OLLAMA.md` files contribute to
...
- **`/settings`**
  - **Description:** Open the settings editor to view and modify Ollama CLI
    settings.
  - **Details:** This command provides a user-friendly interface for changing
    settings that control the behavior and appearance of Ollama CLI. It is
    equivalent to manually editing the `.ollama/settings.json` file, but with
...
- **`/stats`**
  - **Description:** Display detailed statistics for the current Ollama CLI
...
- **`/vim`**
  ...
  - **Features:**
    ...
    - **Persistent setting:** Vim mode preference is saved to
      `~/.ollama/settings.json` and restored between sessions
  - **Status indicator:** When enabled, shows `[NORMAL]` or `[INSERT]` in the
    footer

- **`/init`**
  - **Description:** To help users easily create a `OLLAMA.md` file, this
...
your prompt to Ollama. These commands include git-aware filtering.
...
- **`@<path_to_file_or_directory>`**
  ...
  - **Details:**
    ...
    - The command uses the `read_many_files` tool internally. The content is
      fetched and then inserted into your query before being sent to the Ollama
      model.
...
- **`@` (Lone at symbol)**
  - **Description:** If you type a lone `@` symbol without a path, the query is
    passed as-is to the Ollama model. This might be useful if you are
...
- If the path specified after `@` is not found or is invalid, an error message
  will be displayed, and the query might not be sent to the Ollama model, or it
...
- **`!<shell_command>`**
  - **Description:** Execute the given `<shell_command>` using `bash` on
    ...
    the terminal.
  - **Examples:**
    - `!ls -la` (executes `ls -la` and returns to Ollama CLI)
    - `!git status` (executes `git status` and returns to Ollama CLI)
  ...
  - **Description:** Typing `!` on its own toggles shell mode.
    - **Entering shell mode:**
      ...
      - While in shell mode, text you type is interpreted directly as a shell
        command.
    - **Exiting shell mode:**
      - When exited, the UI reverts to its standard appearance and normal Ollama
...
- **Environment Variable:** When a command is executed via `!` or in shell mode,
  the `OLLAMA_CLI=1` environment variable is set in the subprocess's
