...
prompts as personal shortcuts within Ollama CLI. You can create commands that
...
Ollama CLI discovers commands from two locations, loaded in a specific order:

1.  **User Commands (Global):** Located in `~/.ollama/commands/`. These commands
    are available in any project you are working on.
2.  **Project Commands (Local):** Located in
    `<your-project-root>/.ollama/commands/`. These commands are specific to the
...
- A file at `~/.ollama/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.ollama/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the Ollama model when the
...
```toml
# In: <project>/.ollama/commands/changelog.toml
...
When a custom command attempts to execute a shell command, Ollama CLI will now
...
````toml
# In: <project>/.ollama/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.ollamaignore` if enabled.
...
```toml
# In: <project>/.ollama/commands/review.toml
...
```bash
mkdir -p ~/.ollama/commands/refactor
touch ~/.ollama/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.ollama/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.ollama/commands/refactor/pure.toml
...
Ollama CLI will then execute the multi-line prompt defined in your TOML file.
