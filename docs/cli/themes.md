...
Ollama CLI supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available Themes

Ollama CLI comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Ollama CLI:
...
Selected themes are saved in Ollama CLI's
...
Ollama CLI allows you to create your own custom color themes by specifying them
...
**Security Note:** For your safety, Ollama CLI will only load theme files that
...
- Select your custom theme using the `/theme` command in Ollama CLI. Your custom
