...
Ollama CLI uses `settings.json` files for persistent configuration. There are
three locations for these files:

- **User settings file:**
  - **Location:** `~/.ollama/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all Ollama CLI sessions for the current user.
...
- **System settings file:**
  ...
  - **Scope:** Applies to all Ollama CLI sessions on the system, for all users.
    ...
    administrators at enterprises to have controls over users' Ollama CLI
    setups.
...
- **`theme`** (string):
  - **Description:** Sets the visual [theme](./themes.md) for Ollama CLI.
  - **Default:** `"Default"`
