# Ollama CLI for the Enterprise

This document outlines configuration patterns and best practices for deploying
and managing Ollama CLI in an enterprise environment. By leveraging system-level
...
> using Ollama CLI. However, they should not be considered a foolproof security
...
2. User Settings (`~/.ollama/settings.json`)
3. Workspace Settings (`<project>/.ollama/settings.json`)
...
- **System Defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/ollama-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.ollama/settings.json`):**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["~/ollama-context"]
    }
  }
  ```

- **Workspace `settings.json` (`<project>/.ollama/settings.json`):**
...
- **System Overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/ollama-cli/global-context"]
    }
  }
  ```
...
- **Final Merged Configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/ollama-cli/common-context",
        "~/ollama-context",
        "./project-context",
        "/etc/ollama-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/ollama-cli/settings.json`
  - **Windows**: `C:\ProgramData\ollama-cli\settings.json`
  - **macOS**: `/Library/Application Support/OllamaCli/settings.json`
  - The path can be overridden using the `OLLAMA_CLI_SYSTEM_SETTINGS_PATH`
...
You can significantly enhance security by controlling which tools the Ollama
...
Ollama CLI loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure Ollama
...
For auditing and monitoring purposes, you can configure Ollama CLI to send
...
patterns discussed above to create a secure, controlled environment for Ollama
CLI.
...
```json
{
  ...
  "mcpServers": {
    "corp-tools": {
      "command": "/opt/ollama-tools/start.sh",
      "timeout": 5000
    }
  },
  "telemetry": {
