# Sandboxing in the Ollama CLI

This document provides a guide to sandboxing in the Ollama CLI, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up the Ollama CLI:
...
ollama -s -p "analyze the code structure"

# Use environment variable
export OLLAMA_SANDBOX=true
ollama -p "run the test suite"
...
2. **Environment variable**: `OLLAMA_SANDBOX=true|docker|podman|sandbox-exec`
...
```bash
DEBUG=1 ollama -s -p "debug command"
```
...
```bash
# Check environment
ollama -s -p "run shell command: env | grep SANDBOX"

# List mounts
ollama -s -p "run shell command: mount | grep workspace"
```
