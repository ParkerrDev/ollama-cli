# Provide Context with OLLAMA.md Files

Context files, which use the default name `OLLAMA.md`, are a powerful feature
for providing instructional context to the Ollama model. You can use these files
...
1.  **Global context file:**
    - **Location:** `~/.ollama/OLLAMA.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `OLLAMA.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `OLLAMA.md` files in subdirectories
...
### Example `OLLAMA.md` file

Here is an example of what you can include in a `OLLAMA.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `OLLAMA.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.ollama/OLLAMA.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `OLLAMA.md` files into smaller, more manageable
...
**Example `OLLAMA.md` with imports:**
...
While `OLLAMA.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "OLLAMA.md"]
  }
}
```
