# Getting Started with Ollama CLI Extensions

This guide will walk you through creating your first Ollama CLI extension.
You'll learn how to set up a new extension, add a custom tool via an MCP server,
create a custom command, and provide context to the model with a `OLLAMA.md`
file.

## Prerequisites

Before you start, make sure you have the Ollama CLI installed and a basic
...
```bash
ollama extensions new my-first-extension mcp-server
```
...
This is the manifest file for your extension. It tells Ollama CLI how to load
...
- `mcpServers`: This section defines one or more Model Context Protocol (MCP)
  ...
  - `command`, `args`, `cwd`: These fields specify how to start your server.
    Notice the use of the `${extensionPath}` variable, which Ollama CLI replaces
...
link the extension to your Ollama CLI installation for local development.
...
2.  **Build the server:**
    ...
    referenced in your `ollama-extension.json`.

3.  **Link the extension:**

    The `link` command creates a symbolic link from the Ollama CLI extensions
    ...
    ```bash
    ollama extensions link .
    ```

Now, restart your Ollama CLI session. The new `fetch_posts` tool will be
...
After saving the file, restart the Ollama CLI. You can now run
`/fs:grep-code "some pattern"` to use your new command.

## Step 5: Add a Custom `OLLAMA.md`

You can provide persistent context to the model by adding a `OLLAMA.md` file to
...
1.  Create a file named `OLLAMA.md` in the root of your extension directory:
...
2.  Update your `ollama-extension.json` to tell the CLI to load this file:
...
Restart the CLI again. The model will now have the context from your `OLLAMA.md`
...
You've successfully created a Ollama CLI extension! You learned how to:
...
capabilities into the Ollama CLI.
