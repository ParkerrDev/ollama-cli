...
can install your extension using `ollama extensions install <your-repo-uri>`.
...
version in the `ollama-extension.json` file.
...
way `ollama extensions install <your-repo-uri> --ref=stable`. Or, you could make
...
`ollama extensions install <your-repo-uri>` is to be on the stable release
branch.
...
Ollama CLI extensions can be distributed through
...
When checking for updates, ollama will just look for the "latest" release on
...
To ensure Ollama CLI can automatically find the correct release asset for each
...
requirements - specifically the `ollama-extension.json` file must be at the root
of the archive.
...
Ollama CLI extension for multiple platforms:
