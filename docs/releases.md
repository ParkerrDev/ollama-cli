...
package names beginning with `@google-ollama/**` instead of `@google/**`.
...
[maintainer repo guide](https://github.com/google-ollama/maintainers-ollama-cli/blob/main/npm.md)

### Package scopes

| Package    | `prod` (Wombat Dressing Room) | `dev` (Github Private NPM Repo)           |
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @google/ollama-cli            | @google-ollama/ollama-cli                 |
| Core       | @google/ollama-cli-core       | @google-ollama/ollama-cli-core A2A Server |
| A2A Server | @google/ollama-cli-a2a-server | @google-ollama/ollama-cli-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `ollama-cli`,
`ollama-cli-core` and `ollama-cli-a2a-server` packages, pointing the specified
...
- `npx -y @google/ollama-cli@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y @google/ollama-cli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall @google/ollama-cli && npm uninstall -g @google/ollama-cli && npm cache clean --force &&  npm install @google/ollama-cli@<version>`
...
1.  Go to the
    [Actions tab](https://github.com/google-ollama/ollama-cli/actions/workflows/release-manual.yml)
    of the repository.
...
`packages/cli/google-ollama-cli-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the
  `@google/ollama-cli-core` and `@google/ollama-cli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g @google/ollama-cli` will download these packages, and
  `npm` will handle installing the `@google/ollama-cli-core` dependency
...
1.  **The JavaScript Bundle is Created:**
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      `ollama.js`). The `node-pty` library is excluded from this bundle as it
...
2.  **The `bundle` Directory is Assembled:**
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `ollama.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `ollama.js` (from esbuild) -> `bundle/ollama.js`
...
3.  **The GitHub Release is Created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `ollama.js` executable, are attached as assets to a new GitHub Release.
    - **Why:** This makes the single-file version of the CLI available for
      direct download and enables the
      `npx https://github.com/google-ollama/ollama-cli` command, which downloads
...
- **GitHub Release:** Publishes a single, bundled `ollama.js` file that contains
