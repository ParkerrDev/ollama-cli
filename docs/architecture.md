# Ollama CLI Architecture Overview

This document provides a high-level overview of the Ollama CLI's architecture.

## Core components

The Ollama CLI is primarily composed of two main packages, along with a suite of
tools that can be used by the system in the course of handling command-line
input:

1.  **CLI package (`packages/cli`):**
    - **Purpose:** This contains the user-facing portion of the Ollama CLI, such
...
2.  **Core package (`packages/core`):**
    - **Purpose:** This acts as the backend for the Ollama CLI. It receives
      requests sent from `packages/cli`, orchestrates interactions with the
      Ollama API, and manages the execution of available tools.
    - **Key functions contained in the package:**
      - API client for communicating with the Google Ollama API
...
3.  **Tools (`packages/core/src/tools/`):**
    - **Purpose:** These are individual modules that extend the capabilities of
      the Ollama model, allowing it to interact with the local environment
...
A typical interaction with the Ollama CLI follows this flow:
...
3.  **Request processed:** The core package:
    - Constructs an appropriate prompt for the Ollama API, possibly including
      conversation history and available tool definitions.
    - Sends the prompt to the Ollama API.
4.  **Ollama API response:** The Ollama API processes the prompt and returns a
    response. This response might be a direct answer or a request to use one of
    the available tools.
5.  **Tool execution (if applicable):**
    - When the Ollama API requests a tool, the core package prepares to execute
      it.
    ...
    - Once confirmed, or if confirmation is not required, the core package
      ...
      sent back to the Ollama API by the core package.
