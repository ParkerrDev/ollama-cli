...
This document describes the `web_fetch` tool for the Ollama CLI.
...
## How to use `web_fetch` with the Ollama CLI

To use `web_fetch` with the Ollama CLI, provide a natural language prompt that
contains URLs. The tool will ask for confirmation before fetching any URLs. Once
confirmed, the tool will process URLs through Ollama API's `urlContext`.

If the Ollama API cannot access the URL, the tool will fall back to fetching
...
- **URL processing:** `web_fetch` relies on the Ollama API's ability to access
