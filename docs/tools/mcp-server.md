# MCP servers with the Ollama CLI
...
An MCP server is an application that exposes tools and resources to the Ollama
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the Ollama model
and your local environment or other services like APIs.

An MCP server enables the Ollama CLI to:
...
- **Access resources:** Read data from specific resources (though the Ollama CLI
  primarily focuses on tool execution).

With an MCP server, you can extend the Ollama CLI's capabilities to perform
actions beyond its built-in features, such as interacting with databases, APIs,
custom scripts, or specialized workflows.

## Core Integration Architecture

The Ollama CLI integrates with MCP servers through a sophisticated discovery and
...
4. **Sanitizes and validates** tool schemas for compatibility with the Ollama
   API
...
The Ollama CLI supports three MCP transport types:
...
The Ollama CLI uses the `mcpServers` configuration in your `settings.json` file
...
The Ollama CLI supports OAuth 2.0 authentication for remote MCP servers using
...
- **Stored securely** in `~/.ollama/mcp-oauth-tokens.json`
...
When the Ollama CLI starts, it performs MCP server discovery through the
...
4. **Name sanitization:** Tool names are cleaned to meet Ollama API
   requirements:
...
When the Ollama model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the Ollama model like built-in
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for Ollama API compatibility
...
Ollama CLI's capabilities while maintaining security, reliability, and ease of
use.
...
The Ollama CLI will correctly process this array, separating text from binary
...
When the Ollama CLI receives this response, it will:
...
context to the Ollama model.

## MCP Prompts as Slash Commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within the Ollama CLI. This allows you to create
...
```bash
/poem-writer --title="Ollama CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Ollama CLI" reverent
```

When you run this command, the Ollama CLI executes the `prompts/get` method on
...
## Managing MCP Servers with `ollama mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, the Ollama CLI provides a convenient set of commands to
...
### Adding a Server (`ollama mcp add`)
...
`~/.ollama/settings.json` or the project config `.ollama/settings.json` file.

**Command:**

```bash
ollama mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
ollama mcp add [options] <name> <command> [args...]

# Example: Adding a local server
ollama mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
ollama mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
ollama mcp add --transport http <name> <url>

# Example: Adding an HTTP server
ollama mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
ollama mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
ollama mcp add --transport sse <name> <url>

# Example: Adding an SSE server
ollama mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
ollama mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing Servers (`ollama mcp list`)
...
### Removing a Server (`ollama mcp remove`)
