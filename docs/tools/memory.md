...
This document describes the `save_memory` tool for the Ollama CLI.

## Description

Use `save_memory` to save and recall information across your Ollama CLI
...
## How to use `save_memory` with the Ollama CLI

The tool appends the provided `fact` to a special `OLLAMA.md` file located in
the user's home directory (`~/.ollama/OLLAMA.md`). This file can be configured
to have a different name.

Once added, the facts are stored under a `## Ollama Added Memories` section.
...
```
save_memory(fact="The project I'm currently working on is called 'ollama-cli'.")
```
