...
This document describes the `run_shell_command` tool for the Ollama CLI.
...
## How to use `run_shell_command` with the Ollama CLI
...
`settings.json` file or by using the `/settings` command in the Ollama CLI.
...
When an interactive command is running, you can send input to it from the Ollama
...
When `run_shell_command` executes a command, it sets the `OLLAMA_CLI=1`
environment variable in the subprocess's environment. This allows scripts or
tools to detect if they are being run from within the Ollama CLI.
