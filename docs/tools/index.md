...
The Ollama CLI includes built-in tools that the Ollama model uses to interact
...
## Overview of Ollama CLI tools

In the context of the Ollama CLI, tools are specific functions or modules that
the Ollama model can request to be executed. For example, if you ask Ollama to
...
definitions (schemas) to the Ollama model, executes them when requested, and
returns the results to the model for further processing into a user-facing
response.

These tools provide the following capabilities:

- **Access local information:** Tools allow Ollama to access your local file
  system, read file contents, list directories, etc.
- **Execute commands:** With tools like `run_shell_command`, Ollama can run
...
- **Ground responses:** By using tools to fetch real-time or specific local
  data, Ollama's responses can be more accurate, relevant, and grounded in your
  actual context.

## How to use Ollama CLI tools

To use Ollama CLI tools, provide a prompt to the Ollama CLI. The process works
as follows:

1.  You provide a prompt to the Ollama CLI.
2.  The CLI sends the prompt to the core.
3.  The core, along with your prompt and conversation history, sends a list of
    available tools and their descriptions/schemas to the Ollama API.
4.  The Ollama model analyzes your request. If it determines that a tool is
...
6.  The output from the tool is sent back to the Ollama model.
7.  The Ollama model uses the tool's output to formulate its final answer, which
...
in mind. The Ollama CLI will typically:
...
- **Utilize sandboxing:** All tools are subject to restrictions enforced by
  sandboxing (see [Sandboxing in the Ollama CLI](../cli/sandbox.md)). This means
...
## Learn more about Ollama CLI's tools

Ollama CLI's built-in tools can be broadly categorized as follows:
...
- **[MCP servers](./mcp-server.md)**: MCP servers act as a bridge between the
  Ollama model and your local environment or other services like APIs.
