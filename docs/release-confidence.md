...
the Ollama CLI. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y @google/ollama-cli@<tag> --version` must return the
...
- **Setup:**
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @google/ollama-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @google/ollama-cli@preview`
...
- **Basic Prompting:**
  - [ ] Run `ollama "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `ollama`. Ask a follow-up question to test
        context.

- **Piped Input:**
  - [ ] Run `echo "Summarize this" | ollama` and verify it processes stdin.
...
- **Function Calling:**
  - [ ] In interactive mode, ask ollama to "create a file named hello.md with
...
- [ ] Navigate to `go/ollama-cli-offline-evals-dash`.
