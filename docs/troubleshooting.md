...
- **Error: `Failed to login. Message: Request contains an invalid argument`**
  ...
  - Alternatively, you can obtain the Ollama API key from
...
- **Error: Command not found (when attempting to run Ollama CLI with
  `ollama`).**
  - **Cause:** Ollama CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed Ollama CLI:
    - If you installed `ollama` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update Ollama CLI using the command
      `npm install -g @google/ollama-cli@latest`.
    - If you are running `ollama` from source, ensure you are using the correct
      command to invoke it (e.g., `node packages/cli/dist/index.js ...`). To
      update Ollama CLI, pull the latest changes from the repository, and then
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Ollama CLI may attempt operations
...
- **Ollama CLI is not running in interactive mode in "CI" environments**
  - **Issue:** The Ollama CLI does not enter interactive mode (no prompt
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents the Ollama CLI from starting in its
    interactive mode.
  - **Solution:** If the `CI_` prefixed variable is not needed for the CLI to
    ...
    `env -u CI_TOKEN ollama`

- **DEBUG mode not working from project .env file**
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for ollama-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with ollama-cli behavior.
  - **Solution:** Use a `.ollama/.env` file instead, or configure the
    `advanced.excludedEnvVars` setting in your `settings.json` to exclude fewer
    variables.

## Exit Codes

The Ollama CLI uses specific exit codes to indicate the reason for termination.
...
guide_, consider searching the Ollama CLI
[Issue tracker on GitHub](https://github.com/google-ollama/ollama-cli/issues).
